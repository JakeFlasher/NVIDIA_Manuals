---
title: "4. Nsight Compute CLI"
section: "4"
source: "https://docs.nvidia.com/nsight-compute/NsightComputeCli/#nsight-compute-cli"
---

# [4. Nsight Compute CLI](https://docs.nvidia.com/nsight-compute/NsightComputeCli#nsight-compute-cli)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#nsight-compute-cli "Permalink to this headline")

The User Guide for Nsight Compute CLI.

## [4.1. Introduction](https://docs.nvidia.com/nsight-compute/NsightComputeCli#introduction)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#introduction "Permalink to this headline")
...
## [4.2. Quickstart](https://docs.nvidia.com/nsight-compute/NsightComputeCli#quickstart)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#quickstart "Permalink to this headline")
...
Some additional metrics do not follow this structured naming. They are documented in the [Metrics Reference](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#metrics-reference).
...
The report opens in a new document window. For more information about the report, see the [Profiler Report](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report) for collecting profile information through NVIDIA Nsight Compute.

## [4.3. Usage](https://docs.nvidia.com/nsight-compute/NsightComputeCli#usage)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#usage "Permalink to this headline")

### [4.3.1. Modes](https://docs.nvidia.com/nsight-compute/NsightComputeCli#modes)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#modes "Permalink to this headline")

Modes change the fundamental behavior of the command line profiler. Depending on which mode is chosen, different [Command Line Options](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#command-line-options) become available. For example, [Launch](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#command-line-options-launch) is invalid if the _Attach_ mode is selected.
...
### [4.3.2. Multi-Process Support](https://docs.nvidia.com/nsight-compute/NsightComputeCli#multi-process-support)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#multi-process-support "Permalink to this headline")

The NVIDIA Nsight Compute CLI supports profiling multi-process applications.
See [Launch](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#command-line-options-launch) for which command line options are available.

#### [Platform Support](https://docs.nvidia.com/nsight-compute/NsightComputeCli#platform-support)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#platform-support "Permalink to this headline")
...
On x86_64 Linux, launching from 32-bit applications requires you to enable the `support-32bit` option, and the required 32-bit libraries must be installed on your system.
...
#### [MPI Support](https://docs.nvidia.com/nsight-compute/NsightComputeCli#mpi-support)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#mpi-support "Permalink to this headline")
...
#### [Mandatory Concurrent Kernels](https://docs.nvidia.com/nsight-compute/NsightComputeCli#mandatory-concurrent-kernels)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#mandatory-concurrent-kernels "Permalink to this headline")

Multi-process/multi-GPU applications that contain mandatory concurrent kernels (e.g., NCCL and NVSHMEM communication kernels)
may be unable to make forward progress when profiled with default settings due to multi-pass data collection and serialization across processes within the same process tree.
...
Additionally, `--lockstep-nvtx-include` and `--lockstep-nvtx-exclude` options can be set to limit
synchronization to specific kernel ranges. See [NVTX Filtering](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#nvtx-filtering) for additional information.

> ```c++
> mpirun <args> ncu <args> --lockstep-kernel-launch --lockstep-nvtx-include nccl/ -o report <app>
> ```

[--communicator tcp](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#launch) internally uses TCP sockets in a client-server architecture to send and receive message. One process
at random is selected as the server process to manage sockets.
In a multi-node system, the option `--communicator-tcp-hostname` must be set to manually select one host as the server.
...
You can use the [Report Merge Tool](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#report-merge-tool) and [Clustering Window](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#clustering-window) to aggregate and/or compare these results.

**For communication kernels launched from the same process tree, use --communicator shmem**

In single-node scenarios where the entire process tree is profiled by the same instance of `ncu`, the [--communicator shmem](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#launch) option can be used.
...
This mode only supports _kernel_ and _range_ replay modes and has limited filter options.
...
By default, the first CUDA context of each group is profiled. The `--devices` option can be used to select the primary GPU device(s) to profile.

Setting `--communicator-shmem-num-peers` to the number of processes participating in the concurrent workload `-k ...` is not required but recommended.
...
Launcher processes like `mpirun` or `torchrun` are auto-excluded from profiling.
You can manually exclude others using the [–target-processes-filter](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#launch) option.

### [4.3.3. Output Pages](https://docs.nvidia.com/nsight-compute/NsightComputeCli#output-pages)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#output-pages "Permalink to this headline")
...
### [4.3.4. Profile Import](https://docs.nvidia.com/nsight-compute/NsightComputeCli#profile-import)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#profile-import "Permalink to this headline")

Using the `--import` option, saved reports can be imported into the command line profiler. When using this flag, most other options are not available, except for certain result filterting options. They are marked as such in the [Profile options](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#command-line-options-profile) table.

### [4.3.5. Filtered Profile Export](https://docs.nvidia.com/nsight-compute/NsightComputeCli#filtered-profile-export)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#filtered-profile-export "Permalink to this headline")

Using the `--import` and `--export` options together, along with supported filtering options, you can export desired results from one report to another.
Most of the filtering [Profile options](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#command-line-options-profile) that can be used with `--import` alone are also supported here, except for `--metrics` and `--section`.

### [4.3.6. Metrics and Units](https://docs.nvidia.com/nsight-compute/NsightComputeCli#metrics-and-units)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#metrics-and-units "Permalink to this headline")
...
Both range and domain names can contain whitespace. Note that “Domain” and “range” in below example are for illustration purposes only and are not required to mark domain or range names.
...
### [4.3.8. Config File](https://docs.nvidia.com/nsight-compute/NsightComputeCli#config-file)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#config-file "Permalink to this headline")
...
### [4.3.9. Kernel Renaming](https://docs.nvidia.com/nsight-compute/NsightComputeCli#kernel-renaming)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#kernel-renaming "Permalink to this headline")
...
To see original kernel demangled names, disable kernel renaming using `--rename-kernels off` option.
...
| By default, kernel renaming config file with name `ncu-kernel-renames.yaml` is searched in the similar way as [config file](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#config-file) is searched.
...
If you want to see the original demangled name in the report, you can always rename it again using the renaming configuration file or simply disable kernel renaming using `--rename-kernels off` option.
Note that the renaming can be done on demangled names only. That’s why you need to use `--kernel-name-base demangled` option to use the renamed demangled names for kernel filtering.

You can also use the kernel renaming configuration file while exporting kernels from one report to another using [Filtered Profile Export](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#filtered-profile-export) feature.
...
### [4.3.10. CPU Call Stack Filtering](https://docs.nvidia.com/nsight-compute/NsightComputeCli#cpu-call-stack-filtering)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#cpu-call-stack-filtering "Permalink to this headline")

CPU call stack filtering allows you to filter the launches based on their CPU call stacktrace. You can collect two types of CPU call stacks: “Native” and “Python” using the `--call-stack` or `--call-stack-type` option.
...
You can use each option multiple times, mentioning all the “include” and “exclude” configurations as needed. In order to use these include/exclude options, you must collect the respective call stack type using the `--call-stack` or `--call-stack-type` option.
...
- <File>@<Function> usage is similar to the usage of <Domain>@<Range> in [NVTX Filtering](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#nvtx-filtering)
...
Look at [NVTX Push/Pop Range Filtering](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#nvtx-filtering) section to understand the function expression format in detail.

## [4.4. Command Line Options](https://docs.nvidia.com/nsight-compute/NsightComputeCli#command-line-options)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#command-line-options "Permalink to this headline")

For long command line options, passing a unique initial substring can be sufficient.

### [4.4.1. General](https://docs.nvidia.com/nsight-compute/NsightComputeCli#general)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#general "Permalink to this headline")
...
### [4.4.2. Launch](https://docs.nvidia.com/nsight-compute/NsightComputeCli#launch)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#launch "Permalink to this headline")
...
| call-stack-type | Set the call stack type(s) that should be collected. More than one type may be specified. Implies *–call-stack*. Available modes are:   - **native:** Collect a regular CPU call stack. - **python:** Collect a Python CPU call stack.   Note that Python call stack collection requires CPython version 3.9 or later. | native   **Examples**   `--call-stack-type native --call-stack-type python` |
| communicator | Enable Multiprocess Communicator.   - **none:** Disables the communicator. - **tcp:** TCP-based inter-process tree communication for multiple ncu instances. - **shmem:** Shared memory-based intra-process tree communication for one ncu instance. | none |
...
### [4.4.3. Attach](https://docs.nvidia.com/nsight-compute/NsightComputeCli#attach)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#attach "Permalink to this headline")
...
| c,launch-count | Limit the number of profiled kernel launches. The count is only incremented for launches that match the kernel filters.[^1] |  |
| s,launch-skip | Set the number of kernel launches to skip before starting to profile kernels. The number takes into account only launches that match the kernel filters. [^1] | 0 |
| launch-skip-before-match | Set the number of kernel launches to skip before starting to profile. The count is incremented for all launches, regardless of the kernel filters. [^1] | 0 |
| range-filter | Filter to profile specified instance(s) of matching NVTX ranges or start/stop ranges created through cu(da)ProfilerStart/Stop APIs.   Specify in format _[yes/no/on/off]:[start/stop range instance(s)]:[NVTX range instance(s)]_   - [yes/no/on/off] : default is ‘no/off’. If set to ‘yes/on’ then NVTX range numbering starts from 1 inside every start/stop range. - provide numbers in regex form e.g, [2-4] or 2\|3\|4 to profile 2nd, 3rd and 4th instance of the matching range. - NVTX range numbers will be counted for matching range provided using –nvtx-include. | **Examples**   `--range-filter :2:3 --nvtx-include A/` Match 2nd start/stop range and also 3rd NVTX push/pop range A in the app.   `--range-filter yes:2:3 --nvtx-include A/` Match 3rd NVTX push/pop range A from 2nd start/stop range. |
| kill | Terminate the target application when the requested –launch-count was profiled. Allowed values:   - on/off - yes/no | no |
| replay-mode | Mechanism used for replaying a kernel launch multiple times to collect all requested profiling data:   - **kernel:** Replay individual kernel launches “transparently” during the execution of the application. See [Kernel Replay](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#kernel-replay) for more details. - **application:** Relaunch the entire application multiple times. Requires deterministic program execution. See [Application Replay](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#application-replay) for more details. - **range:** Replay ranges of CUDA API calls and kernel launches “transparently” during the execution of the application. Ranges must be defined using `cu(da)ProfilerStart/Stop` API pairs or [NVTX expressions](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#nvtx-filtering). See [Range Replay](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#range-replay) for more details. - **app-range:** Profile ranges without API capture by relaunching the entire application multiple times. Requires deterministic program execution. Ranges must be defined using `cu(da)ProfilerStart/Stop` API pairs or [NVTX expressions](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#nvtx-filtering). See [Application Range Replay](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#application-range-replay) for more details. | kernel |
| app-replay-buffer | Application replay buffer location.   - **file:** Replay pass data is buffered in a temporary file. The report is created after profiling completed. This mode is more scalable, as the amount of required memory does not scale with the number of profiled kernels. - **memory:** Replay pass data is buffered in memory, and the report is created while profiling. This mode can result in better performance if the filesystem is slow, but the amount of required memory scales with the number of profiled kernels. | file |
| app-replay-match | Application replay kernel matching strategy. For all options, kernels are matched on a per-process and per-device (GPU) basis. Below options are used to configure the applied strategy in more detail.   - **name:** Kernels are matched in the following order: 1. (mangled) name, 2. order of execution - **grid:** Kernels are matched in the following order: 1. (mangled) name, 2. CUDA grid/block size, 3. order of execution - **all:** Kernels are matched in the following order: 1. (mangled) name, 2. CUDA grid/block size, 3. CUDA context ID, 4. CUDA stream ID, 5. order of execution | grid |
| app-replay-mode | Application replay kernel matching mode:   - **strict:** Requires all filtered kernels to match across all replay passes in the exact order. - **balanced:** Requires all filtered kernels to match across all replay passes, without strict ordering. - **relaxed:** Produces results only for filtered kernels that could be matched across replay passes. Unmatched kernels are dropped. | balanced |
| range-replay-options | Range replay options, separated by comma. Below options are supported:   - **enable-greedy-sync**  Insert ctx sync for applicable deferred APIs during capture. - **disable-host-save**  Disable saving device-mapped host allocations for every kernel but capture it only once before the first kernel launch. - **disable-host-restore**  Disable restoring device-written host allocations. - **disable-dependent-kernel-detection**  Disable dependent kernel detection before capturing device-mapped host allocations for each kernel. | none |
| graph-profiling | CUDA graph profiling mode:   - **node**  Profile individual kernel nodes as regular CUDA kernels. - **graph**  Profile entire graphs as one workload (but disable profiling of individual graph kernel nodes). See the [Profiling Guide](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#graph-profiling) for more information on this mode. | node |
...
| apply-rules | Apply active and applicable rules to each profiling result. Use `--rule` to limit which rules to apply. Allowed values:   - on/off - yes/no | yes |
...
| import-sass | Import ELF cubins into the report. Cubins include SASS, PTX and meta information. Allowed values:   - on/off - yes/no   Disabling this is not recommended, as it removes most source-related metrics and analysis. | yes |
| import-source | If available from -lineinfo, correlated CUDA source files are permanently imported into the report. Allowed values:   - on/off - yes/no   Use `--source-folders` option to provide missing source files. | no |
...
| profile-from-start | Set if application should be profiled from its start. Allowed values:   - on/off - yes/no | yes |
...
| clock-control | Control the behavior of the GPU clocks during profiling. Allowed values:   - **base:** GPC and memory clocks are locked to their respective base (rated tdp) frequency during profiling. - **boost:** GPC and memory clocks are locked to their respective turbo boost frequency during profiling. If setting boost clock is not supported by GPU or driver, falls back to locking clocks to ‘base’ if possible. - **force-boost:** GPC and memory clocks are locked to their respective turbo boost frequency during profiling. Supports Ampere+ on NVIDIA kernel mode driver 560 and Turing+ on driver 580 or newer. - **none:** No GPC or memory frequencies are changed during profiling. - **reset:** Reset GPC and memory clocks for all or the selected devices and exit. Use if a previous, killed execution of ncu left the GPU clocks in a locked state.   This has no impact on thermal throttling. Note that actual clocks might still vary, depending on the level of driver support for this feature. As an alternative, use `nvidia-smi` to lock the clocks externally and set this option to `none`. | boost |
| pipeline-boost-state | Control the Tensor Core boost state. Setting stable Tensor Core boosting is recommended for application performance profiling ensuring predictive run to run performance. Allowed values:   - **stable:** Set the Tensor Core boost state to stable. - **dynamic:** Set the Tensor Core boost state to dynamic. | stable |
| nvtx-include | Adds an include statement to the [NVTX filter](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#nvtx-filtering), which allows selecting kernels to profile based on NVTX ranges. [^1] |  |
| nvtx-exclude | Adds an exclude statement to the [NVTX filter](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#nvtx-filtering), which allows selecting kernels to profile based on NVTX ranges. [^1] |  |
| native-include | Adds an include statement to the [Native call stack filtering](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#cpu-stack-filtering), which allows selecting kernels to profile based on native CPU stack frames. [^1] |  |
| native-exclude | Adds an exclude statement to the [Native call stack filtering](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#cpu-stack-filtering), which allows selecting kernels to profile based on native CPU stack frames. [^1] |  |
| python-include | Adds an include statement to the [Python call stack filtering](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#cpu-stack-filtering), which allows selecting kernels to profile based on python CPU stack frames. [^1] |  |
| python-exclude | Adds an exclude statement to the [Python call stack filtering](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#cpu-stack-filtering), which allows selecting kernels to profile based on python CPU stack frames. [^1] |  |

**1([1](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id7),[2](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id8),[3](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id9),[4](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id10),[5](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id11),[6](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id12),[7](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id13),[8](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id14),[9](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id15),[10](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id16),[11](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id17),[12](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id18),[13](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id19),[14](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id20),[15](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#id21))**

  This filtering option is available when using `--import`.

### [4.4.5. PM Sampling](https://docs.nvidia.com/nsight-compute/NsightComputeCli#pm-sampling)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#pm-sampling "Permalink to this headline")

These options apply to [PM sampling](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#pm-sampling). See [here](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#warp-sampling) for options used in warp state sampling.
...
### [4.4.6. Warp Sampling](https://docs.nvidia.com/nsight-compute/NsightComputeCli#warp-sampling)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#warp-sampling "Permalink to this headline")

These options apply to [Warp State Sampling](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#statistical-sampler). See [here](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#pm-sampling) for options used in PM sampling.
...
| warp-sampling-max-passes | Set maximum number of passes used for sampling (see the [Profiling Guide](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#overhead) for more details on profiling overhead). | 5 |
| warp-sampling-buffer-size | Set the size of the device-sided allocation for samples in bytes. | 32*1024*1024 |

### [4.4.7. File](https://docs.nvidia.com/nsight-compute/NsightComputeCli#file)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#file "Permalink to this headline")
...
| o,export | Set the output file for writing the profile report. If not set, a temporary file will be used which is removed afterwards. Use with `--import` option to save filtered results. See [Filtered Profile Export](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#filtered-profile-export) for more details. The specified name supports macro expansion. See [File Macros](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#command-line-options-file-macros) for more details. | If `--export` is set and no `--page` option is given, no profile results will be printed on the console. |
...
| print-metric-instances | Set output mode for metrics with instance values:   - **none** Only show GPU aggregate value. - **values** Show GPU aggregate followed by all instance values. - **details** Show aggregate value, followed by correlation IDs and instance values | none |
| print-nvtx-rename | Select how NVTX should be used for renaming:   - **none** Don’t use NVTX for renaming. - **kernel** Rename kernels with the most recent enclosing NVTX push/pop range. | none |
...
| print-summary | Select the summary output mode. Available modes are:   - **none** No summary. - **per-gpu** Summary for each GPU. - **per-kernel** Summary for each kernel type. - **per-nvtx** Summary for each NVTX context. | none |

### [4.4.9. Response File](https://docs.nvidia.com/nsight-compute/NsightComputeCli#response-file)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#response-file "Permalink to this headline")
...
### [4.4.10. File Macros](https://docs.nvidia.com/nsight-compute/NsightComputeCli#file-macros)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#file-macros "Permalink to this headline")
...
### [4.4.11. MPS](https://docs.nvidia.com/nsight-compute/NsightComputeCli#mps)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#mps "Permalink to this headline")

| Option | Description | Default |
| --- | --- | --- |
| mps | Select MPS behavior. Available modes are:   > - **none** No MPS support > - **client** Launch MPS client process > - **primary-client** Launch primary MPS client process > - **control** Launch MPS ncu control | none |
| mps-num-clients | Number of MPS client processes. |  |
| mps-timeout | Timeout (seconds) to discover MPS client processes. |  |

| mps-timeout | mps-num-clients | Behavior |
| --- | --- | --- |
...
## [4.5. Environment Variables](https://docs.nvidia.com/nsight-compute/NsightComputeCli#environment-variables)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#environment-variables "Permalink to this headline")
...
## [4.6. Nvprof Transition Guide](https://docs.nvidia.com/nsight-compute/NsightComputeCli#nvprof-transition-guide)[](https://docs.nvidia.com/nsight-compute/NsightComputeCli/#nvprof-transition-guide "Permalink to this headline")

Refer to the [older documentation](https://docs.nvidia.com/nsight-compute/Archives/index.html).
