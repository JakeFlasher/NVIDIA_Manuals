---
title: "3. Nsight Compute"
section: "3"
source: "https://docs.nvidia.com/nsight-compute/NsightCompute/#nsight-compute"
---

# [3. Nsight Compute](https://docs.nvidia.com/nsight-compute/NsightCompute#nsight-compute)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#nsight-compute "Permalink to this headline")

The User Guide for Nsight Compute.

## [3.1. Introduction](https://docs.nvidia.com/nsight-compute/NsightCompute#introduction)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#introduction "Permalink to this headline")

NVIDIA Nsight Compute (UI) User Manual: Detailed documentation on views, controls, and workflows.

### [3.1.1. Overview](https://docs.nvidia.com/nsight-compute/NsightCompute#overview)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#overview "Permalink to this headline")
...
## [3.2. Quickstart](https://docs.nvidia.com/nsight-compute/NsightCompute#quickstart)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#quickstart "Permalink to this headline")
...
![../_images/quick-start-interactive-profiling-connect.png](images/nsight-compute_2.png)
...
![../_images/quick-start-interactive-profiling-attach.png](images/nsight-compute_3.png)
...
![../_images/quick-start-interactive-profiling-connected.png](images/nsight-compute_4.png)
...
![../_images/quick-start-interactive-profiling-api-stream.png](images/nsight-compute_5.png)
...
![../_images/quick-start-interactive-profiling-next-launch.png](images/nsight-compute_6.png)
6. **Profile a kernel launch**

Once the execution of the target application is suspended at a kernel launch, additional actions become available in the UI. These actions are either available from the menu or from the toolbar. Please note that the actions are disabled, if the API stream is not at a qualifying state (not at a kernel launch or launching on an unsupported GPU). To profile, press _Profile Kernel_ and wait until the result is shown in the [Profiler Report](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report). Profiling progress is reported in the lower right corner status bar.
...
For a detailed description of the options available in this activity, see [Interactive Profile Activity](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#connection-activity-interactive).

### [3.2.2. Non-Interactive Profile Activity](https://docs.nvidia.com/nsight-compute/NsightCompute#non-interactive-profile-activity)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#non-interactive-profile-activity "Permalink to this headline")
...
![../_images/quick-start-system-trace-connect.png](images/nsight-compute_9.png)
2. **Additional Launch Options**
...
![../_images/quick-start-system-trace-options.png](images/nsight-compute_10.png)
3. Once the session is completed, the [Nsight Systems](https://docs.nvidia.com/nsight-systems/UserGuide/index.html) report is opened in a new document. By default, the timeline view is shown. It provides detailed information of the activity of the CPU and GPUs and helps understanding the overall behavior and performance of application. Once a CUDA kernel is identified to be on the critical path and not meeting the performance expectations, right click on the kernel launch on timeline and select _Profile Kernel_ from the context menu. A new [Start Activity](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#connection-dialog) opens up that is already preconfigured to profile the selected kernel launch. Proceed with optimizing the selected kernel using [Non-Interactive Profile Activity](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#quick-start-non-interactive)

![../_images/quick-start-system-trace-timeline.png](images/nsight-compute_11.png)

### [3.2.4. Navigate the Report](https://docs.nvidia.com/nsight-compute/NsightCompute#navigate-the-report)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#navigate-the-report "Permalink to this headline")
...
> ![../_images/quick-start-report-summary.png](images/nsight-compute_12.png)

You can switch between different [Report Pages](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-pages) using the tab bar on the top-left of the report.
You can also use _Ctrl + Shift + N_ and _Ctrl + Shift + P_ shortcut keys or corresponding toolbar button to navigate next and previous pages, respectively.
A report can contain any number of results. The _Current_ dropdown allows switching between the different results in a report.

![../_images/quick-start-report.png](images/nsight-compute_13.png)
2. **Diffing multiple results**

On the _Details_ page, use the _Compare - Add Baseline_ button for the current result to become the baseline all other results from this report and any other report opened in the same instance of NVIDIA Nsight Compute get compared to.
...
![../_images/quick-start-baseline.png](images/nsight-compute_14.png)

Use the _Clear Baselines_ entry from the same group button, the Profile menu or the corresponding toolbar button to remove all baselines. For more information see [Baselines](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#baselines).
3. **Following rules**

On the _Details_ page, many sections provide rules with valuable information on detected problems and optimization suggestions.
Rules can be user-defined too. For more information, see the [Customization Guide](https://docs.nvidia.com/nsight-compute/CustomizationGuide/index.html#rule-system).

![../_images/quick-start-rule.png](images/nsight-compute_15.png)

## [3.3. Start Activity Dialog](https://docs.nvidia.com/nsight-compute/NsightCompute#start-activity-dialog)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#start-activity-dialog "Permalink to this headline")
...
![../_images/connection-dialog.png](images/nsight-compute_16.png)
...
### [3.3.1. Remote Connections](https://docs.nvidia.com/nsight-compute/NsightCompute#remote-connections)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#remote-connections "Permalink to this headline")
...
![../_images/add-remote-connection.png](images/nsight-compute_17.png)
...
![../_images/add-remote-connection-private-key.png](images/nsight-compute_18.png)
  - **IP/Host Name:** The IP address or host name of the target device.
  - **User Name:** The user name to be used for the SSH connection.
...
![../_images/progress-log.png](images/nsight-compute_19.png)
...
### [3.3.2. Interactive Profile Activity](https://docs.nvidia.com/nsight-compute/NsightCompute#connection-activity-interactive)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#connection-activity-interactive "Permalink to this headline")
...
For _Base_, GPC and memory clocks are locked to their respective base frequency during profiling. This has no impact on thermal throttling.
For _Boost_, GPC and memory clocks are locked to their respective turbo boost frequency during profiling. If setting boost clock is not supported by the GPU or driver, falls back to locking clocks to ‘base’ if possible.
For _Force-Boost_, GPC and memory clocks are locked to their respective turbo boost frequency during profiling. Supports Ampere+ on NVIDIA kernel mode driver 560 and Turing+ on driver 580 or newer.
For _None_, no GPC or memory frequencies are changed during profiling.
- **Import Source**

Enables permanently importing available source files into the report. Missing source files are searched in [Source Lookup](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-source-lookup) folders. Source information must be embedded in the executable, e.g. via the `-lineinfo` compiler option. Imported files are used in the _CUDA-C_ view on the [Source Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-source-page).

- **Graph Profiling**

Set if CUDA graphs should be stepped and profiled as individual _Nodes_ or as complete _Graphs_. See the [Profiling Guide](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#graph-profiling) for more information on this mode.
...
### [3.3.3. Profile Activity](https://docs.nvidia.com/nsight-compute/NsightCompute#profile-activity)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#profile-activity "Permalink to this headline")
...
All remaining options map to their command line profiler equivalents. See the [Command Line Options](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#command-line-options) for details.

### [3.3.4. Reset](https://docs.nvidia.com/nsight-compute/NsightCompute#reset)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#reset "Permalink to this headline")

Entries in the connection dialog are saved as part of the current [project](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#projects). When working in a custom project, simply close the project to reset the dialog.

When not working in a custom project, entries are stored as part of the _default project_. You can delete all information from the default project by closing NVIDIA Nsight Compute and then [deleting the project file from disk](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#projects).

## [3.4. Main Menu and Toolbar](https://docs.nvidia.com/nsight-compute/NsightCompute#main-menu-and-toolbar)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#main-menu-and-toolbar "Permalink to this headline")

Information on the main menu and toolbar.

![../_images/main-menu.png](images/nsight-compute_20.png)

### [3.4.1. Main Menu](https://docs.nvidia.com/nsight-compute/NsightCompute#id2)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#id2 "Permalink to this headline")

- File
  - **New Project** Create new profiling [Projects](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#projects) with the [New Project Dialog](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#projects-dialog).
...
- Connection
  - **Start Activity** Open the [Start Activity Dialog](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#connection-dialog) to launch or attach to a target application. Disabled when already connected.
...
Note that some non-zero return codes are expected and not necessarily errors.
In addition, certain errors may be ignored if they don’t happen on a nested API call, as the top-level call may still handle them.
  - **Run to Next Kernel** See [API Stream](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-api-stream) tool window.
  - **Run to Next API Call** See [API Stream](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-api-stream) tool window.
  - **Run to Next Range Start** See [API Stream](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-api-stream) tool window.
  - **Run to Next Range End** See [API Stream](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-api-stream) tool window.
  - **API Statistics** Opens the [API Statistics](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-api-statistics) tool window
  - **API Stream** Opens the [API Stream](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-api-stream) tool window
  - **Resources** Opens the [Resources](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-resources) tool window
  - **NVTX** Opens the [NVTX](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-nvtx) tool window
- Profile
  ...
  - **Baselines** Opens the [Baselines](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-baselines) tool window.
  ...
  - **Section/Rules Info** Opens the [Metric Selection](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-sections-info) tool window.
  - **Function Stats** Opens the [Function Stats](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-function-stats) tool window.
- Tools
  - **Project Explorer** Opens the [Project Explorer](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#projects-explorer) tool window.
  - **Output Messages** Opens the Output Messages tool window.
  - **Options** Opens the [Options](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options) dialog.
  - **Search** Opens the [Search](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-search) tool window.
- Window
  ...
  - **Show Welcome Page** Opens the [Welcome Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#quick-start__fig-welcome-page).
- Help
...
### [3.4.2. Main Toolbar](https://docs.nvidia.com/nsight-compute/NsightCompute#main-toolbar)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#main-toolbar "Permalink to this headline")

The main toolbar shows commonly used operations from the main menu. See [Main Menu](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#main-menu) for their description.

### [3.4.3. Status Banners](https://docs.nvidia.com/nsight-compute/NsightCompute#status-banners)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#status-banners "Permalink to this headline")
...
![../_images/status-banner.png](images/nsight-compute_21.png)

## [3.5. Tool Windows](https://docs.nvidia.com/nsight-compute/NsightCompute#tool-windows)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#tool-windows "Permalink to this headline")

### [3.5.1. API Statistics](https://docs.nvidia.com/nsight-compute/NsightCompute#api-statistics)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#api-statistics "Permalink to this headline")
...
![../_images/tool-window-api-statistics.png](images/nsight-compute_22.png)
...
### [3.5.2. API Stream](https://docs.nvidia.com/nsight-compute/NsightCompute#api-stream)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#api-stream "Permalink to this headline")
...
![../_images/tool-window-api-stream.png](images/nsight-compute_23.png)
...
### [3.5.3. Baselines](https://docs.nvidia.com/nsight-compute/NsightCompute#baselines)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#baselines "Permalink to this headline")

The _Baselines_ tool window can be opened by clicking the _Baselines_ entry in the _Profile_ menu. It provides a centralized place from which to manage configured baselines. (Refer to [Baselines](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#baselines), for information on how to create baselines from profile results.)

![../_images/tool-window-baselines.png](images/nsight-compute_24.png)
...
### [3.5.4. Metric Details](https://docs.nvidia.com/nsight-compute/NsightCompute#metric-details)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#metric-details "Permalink to this headline")

The _Metric Details_ tool window can be opened using the _Metric Details_ entry in the _Profile_ menu or the respective tool bar button.
...
![../_images/tool-window-metric-details.png](images/nsight-compute_25.png)

Report metrics can be selected on the [Details Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-details-page) or on the [Raw Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-raw-page).
...
For results collected for [Green Contexts](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#cuda-green-contexts) applications,
the _Metric Details_ tool window also shows an additional row with the _Attribution_ level of
...
Some metrics contain [Instance Values](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#metrics-structure). When available, they are listed in the tool window. Instance values can have a _Correlation ID_ that allows correlating the individual value with its associated entity, e.g. a function address or instruction name.

For metrics collected with [PM sampling](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#pm-sampling), the correlation ID is the GPU timestamp in nanoseconds. It is shown relative to the first timestamp for this metric.
Note that the instance values for PM sampling metrics are currently not [context-switched](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#ctx-switch-trace) , even if this is enabled in the corresponding timeline UI.

### [3.5.5. Launch Details](https://docs.nvidia.com/nsight-compute/NsightCompute#launch-details)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#launch-details "Permalink to this headline")

The _Launch Details_ tool window can be opened using the _Launch Details_ entry
in the _Profile_ menu or the respective tool bar button. When a result
...
![../_images/tool-window-launch-details.png](images/nsight-compute_26.png)
...
#### [Header](https://docs.nvidia.com/nsight-compute/NsightCompute#header)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#header "Permalink to this headline")

On the left side of its header, this tool window displays the selected result’s
...
#### [Body](https://docs.nvidia.com/nsight-compute/NsightCompute#body)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#body "Permalink to this headline")
...
- _Aggregate Value_: the aggregate value for this metric over all sub-launches
in the selected result

### [3.5.6. Function Stats](https://docs.nvidia.com/nsight-compute/NsightCompute#function-stats)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#function-stats "Permalink to this headline")

The _Function Stats_ tool window presents a table of all functions in your profile result with top stalls for each function and all stall reasons.
...
#### [Accessing Function Stats](https://docs.nvidia.com/nsight-compute/NsightCompute#accessing-function-stats)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#accessing-function-stats "Permalink to this headline")

**The Function Stats tool window can be opened using:**

  - The _Function Stats_ entry in the _Profile_ menu. See [Main Menu](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#main-menu) for details.
  - The main tool bar button

#### [Basic Workflow](https://docs.nvidia.com/nsight-compute/NsightCompute#basic-workflow)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#basic-workflow "Permalink to this headline")
...
![../_images/tool-window-function-stats.png](images/nsight-compute_27.png)

The Function Stats tool window

#### [Understanding the Function Stats Table](https://docs.nvidia.com/nsight-compute/NsightCompute#understanding-the-function-stats-table)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#understanding-the-function-stats-table "Permalink to this headline")
...
See the _Warp Stall Reasons_ tables in the [Metrics Reference](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#warp-stall-reasons) for a description of the individual warp scheduler states.

#### [Data Source](https://docs.nvidia.com/nsight-compute/NsightCompute#data-source)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#data-source "Permalink to this headline")
...
- At least one **PM Sampling** metric is required with the attribute `PmWarpSampling` declared
(`Visible` or `Hidden`) to enable PM Warp Sampling data collection for this window.
Use:
...
- The feature is **enabled by default** on all GPUs *GA10X* and newer. Disable globally with
...
#### [Metric Naming](https://docs.nvidia.com/nsight-compute/NsightCompute#metric-naming)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#metric-naming "Permalink to this headline")

Warp Stall Sampling metrics are the **same underlying source counter metrics**; they are requested by prefixing the counter name with `warpsampling:` to indicate collection via the PM Sampling path (**PM Warp Sampling**) and to render them on a timeline.
Example:
...
### [3.5.7. NVTX](https://docs.nvidia.com/nsight-compute/NsightCompute#nvtx)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#nvtx "Permalink to this headline")
...
![../_images/tool-window-nvtx.png](images/nsight-compute_28.png)

Use the _Current Thread_ dropdown in the [API Stream](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-api-stream) window to change the currently selected thread. NVIDIA Nsight Compute supports NVTX named resources, such as threads, CUDA devices, CUDA contexts, etc. If a resource is named using NVTX, the appropriate UI elements will be updated.

![../_images/tool-window-nvtx-resources.png](images/nsight-compute_29.png)

### [3.5.8. CPU Call Stack](https://docs.nvidia.com/nsight-compute/NsightCompute#cpu-call-stack)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#cpu-call-stack "Permalink to this headline")

The _CPU Call Stack_ window is available when NVIDIA Nsight Compute is connected to a target application.
If closed, it can be re-opened using _Debug > CPU Call Stack_ from the main menu.
...
![../_images/tool-window-callstack.png](images/nsight-compute_30.png)

Use the _Call Stack Type_ dropdown menu to switch between stack types in case multiple stack types were enabled (e.g., _Native_, _Python_).
Note that Python call stack collection requires CPython version 3.9 or later.

### [3.5.9. Resources](https://docs.nvidia.com/nsight-compute/NsightCompute#resources)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#resources "Permalink to this headline")
...
![../_images/tool-window-resources.png](images/nsight-compute_31.png)
...
#### [Memory Allocations](https://docs.nvidia.com/nsight-compute/NsightCompute#memory-allocations)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#memory-allocations "Permalink to this headline")
...
#### [Graphviz DOT and SVG exports](https://docs.nvidia.com/nsight-compute/NsightCompute#graphviz-dot-and-svg-exports)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#graphviz-dot-and-svg-exports "Permalink to this headline")
...
### [3.5.10. CUDA Graph Viewer](https://docs.nvidia.com/nsight-compute/NsightCompute#cuda-graph-viewer)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#cuda-graph-viewer "Permalink to this headline")
...
If the window is closed, you can reopen it by clicking the _CUDA Graph Viewer_ button in _Resources > Graphs: Graphs_ or in any other _Graphs:_ resource from the main menu. The viewer displays the current state of all active CUDA Graphs whenever the target application is suspended.

![../_images/tool-window-cuda-graph-viewer.png](images/nsight-compute_32.png)
...
Note that the graph layouts are optimized for interactive visualization and may differ from those generated by CUDA API functions such as `cudaGraphDebugDotPrint` or exported as [DOT or SVG](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#graphviz-dot-and-svg-exports) from the _Resources_ window.
The viewer uses hierarchical layout algorithms to improve readability and highlight execution flow.
...
- When a source graph is being built, the _Source Graph_ tab automatically shows the graph in its current state.
As nodes are added through CUDA API calls, the viewer updates in real-time to highlight newly created nodes and their connections.
- When an instantiated graph is launched, the _Instantiated Graph_ tab automatically displays the graph execution state.
As you step through the application, the viewer highlights nodes currently being executed and visually distinguishes completed nodes from pending ones.
Execution flow through conditional graph and child graph nodes is clearly indicated.

### [3.5.11. Search](https://docs.nvidia.com/nsight-compute/NsightCompute#search)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#search "Permalink to this headline")

The _Search_ tool window can be opened using the _Search_ entry in the _Tools_ menu, or from the tool bar’s search bar.
...
![../_images/tool-window-search.png](images/nsight-compute_33.png)

Enter your query into the tool menu’s or window’s search bar and press _Enter_ or use the _Start search_ button.
Available sources are checked for matches in the background and results are displayed in the tool window’s table.
While a search is running, you can press the _Cancel search_ button to stop it.
...
![../_images/tool-window-search-results.png](images/nsight-compute_34.png)

Each result is associated with the original source.
While some results are plain text, others are links that can be clicked to jump to the location and see them in context.
Below is a (non-comprehensive) list of supported sources. The list of available sources may be extended in the future.
...
![../_images/tool-window-section-sets.png](images/nsight-compute_35.png)
...
![../_images/tool-window-sections.png](images/nsight-compute_36.png)
...
Similarly, when a stock section or rule file is removed from the configured _Sections Directory_ (specified in the [Profile](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-profile) options dialog), the _State_ column will show _User Deleted_. _User Deleted_ files can also be restored using the _Restore_ button.
...
See the [Sections and Rules](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#sections-and-rules) for the list of default sections for NVIDIA Nsight Compute.

## [3.6. Profiler Report](https://docs.nvidia.com/nsight-compute/NsightCompute#profiler-report)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#profiler-report "Permalink to this headline")
...
### [3.6.1. Header](https://docs.nvidia.com/nsight-compute/NsightCompute#profiler-report-header)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#profiler-report-header "Permalink to this headline")

![../_images/profiler-report-header.png](images/nsight-compute_37.png)

The top of the report shows a table with information about the selected profile result (as _Current_) and potentially additional [baselines](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-baselines).
For many values in this table, tooltips provide additional information or data, e.g., the tooltip of the column _Attributes_ provides additional information about the context type and resources used for the launch.
...
The _Apply Filters_ button opens the filter dialog. You can use more than one filter to narrow down your results. On the filter dialog, enter your filter parameters and press OK button. The _Launch_ dropdown, [Summary Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-summary-page) table, and [Raw Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-raw-page) table will be filtered accordingly. Select the arrow dropdown to access the _Clear Filters_ button, which removes all filters.

![../_images/profiler-report-header-filter-dialog.png](images/nsight-compute_38.png)

Filter Dialog

Underneath the current and baseline results are the tabs for switching between the report pages. The pages themselves are explained in detail in the [next section](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-pages).
...
- Compare
  ...
  - **Clear Baselines** removes all currently active baselines. You may also use the [Baselines](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-baselines) tool window to manage baseline for comparison.
  - **Source Comparison** navigates to the [Source Comparison](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#source-comparison) document in case at least two profile results are available for comparison.
- Tools
  - **Occupancy Calculator** opens the [Occupancy Calculator](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#occupancy-calculator) in a new document.
  - **Metric Details Windows** opens the [Metric Details](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-metric-details) tool window. When the window is open and a metric is selected elsewhere in the report, it shows detailed information about it.
  - **Launch Details Windows** opens the [Launch Details](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-launch-details) tool window. When the window is open and a result containing multiple sub-launches is selected, it displays information about each sub-launch in the result.
- View
...
### [3.6.2. Report Pages](https://docs.nvidia.com/nsight-compute/NsightCompute#report-pages)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#report-pages "Permalink to this headline")

Use the _Page_ dropdown in the header to switch between the report pages.

By default, when opening a report with a single profile result, the [Details Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-details-page) is shown. When opening a report with multiple results, the [Summary Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-summary-page) is selected instead. You can change the default report page in the [Profile](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-profile) options.

#### [Summary Page](https://docs.nvidia.com/nsight-compute/NsightCompute#summary-page)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#summary-page "Permalink to this headline")
...
Use [Rename Kernels Config Path](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-profile) option to specify the configuration file which should be used while importing renamed kernels or exporting demangled names with mappings to rename them. To export names to a new file, click _Export_ button and use _Rename Kernels Config_ option.
See [Kernel Renaming](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#kernel-renaming) for more details on configuration file usage.

![../_images/profiler-report-pages-summary-table.png](images/nsight-compute_40.png)

You can configure the list of metrics included in this table in the [Profile](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-profile) options dialog. If a metric has multiple instance values, the number of instances is shown after its standard value. A metric with ten instance values could for example look like this: `35.48 {10}`. In the [Profile](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-profile) options dialog, you can select that all instance values should be shown individually. You can also inspect the instances values of a metric result in the [Metric Details](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-metric-details) tool window.

In addition to metrics, you can also configure the table to include any of the following properties:

##### [Properties](https://docs.nvidia.com/nsight-compute/NsightCompute#properties)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#properties "Permalink to this headline")
...
For [Range Replay](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#range-replay) reports, a smaller set of columns is shown by default, as not all apply to such results.
...
![../_images/profiler-report-pages-summary-rules.png](images/nsight-compute_41.png)

_Prioritized Rules_ with key performance indicators table.

#### [Details Page](https://docs.nvidia.com/nsight-compute/NsightCompute#details-page)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#details-page "Permalink to this headline")

##### [Overview](https://docs.nvidia.com/nsight-compute/NsightCompute#id5)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#id5 "Permalink to this headline")

The _Details_ page is the main page for all metric data collected during a kernel launch. The page is split into individual sections.
...
For more information on customizing sections, see the [Customization Guide](https://docs.nvidia.com/nsight-compute/CustomizationGuide/index.html#abstract).
For a list of sections shipped with NVIDIA Nsight Compute, see [Sections and Rules](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#sections-and-rules).
...
![../_images/profiler-report-pages-section-with-rule.png](images/nsight-compute_42.png)
...
![../_images/profiler-report-pages-section-bodies.png](images/nsight-compute_44.png)

Sections with multiple bodies have a dropdown to switch between them.

##### [Memory](https://docs.nvidia.com/nsight-compute/NsightCompute#memory)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#memory "Permalink to this headline")

If enabled, the _Memory Workload Analysis_ section contains a Memory chart that visualizes data transfers, cache hit rates, instructions and memory requests. More information on how to use and read this chart can be found in the [Profiling Guide](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#memory-chart).

##### [Occupancy](https://docs.nvidia.com/nsight-compute/NsightCompute#occupancy)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#occupancy "Permalink to this headline")

You can open the [Occupancy Calculator](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#occupancy-calculator) by clicking on the calculator button in the report header or in the header of the _Occupancy Section_.

##### [Range Replay](https://docs.nvidia.com/nsight-compute/NsightCompute#range-replay)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#range-replay "Permalink to this headline")

Note that for [Range Replay](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#range-replay) results some UI elements, analysis rules, metrics or section body items such as charts or tables might not be available, as they only apply to kernel launch-based results. The filters can be checked in the corresponding section files.

##### [Rooflines](https://docs.nvidia.com/nsight-compute/NsightCompute#rooflines)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#rooflines "Permalink to this headline")

If enabled, the _GPU Speed Of Light Roofline Chart_ section contains a Roofline chart that is particularly helpful for visualizing kernel performance at a glance. (To enable roofline charts in the report, ensure that the section is enabled when profiling.) More information on how to use and read this chart can be found in [Roofline Charts](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#roofline). NVIDIA Nsight Compute ships with several different definitions for roofline charts, including hierarchical rooflines. These additional rooflines are defined in different section files. While not part of the _full_ section set, a new section set called _roofline_ was added to collect and show all rooflines in one report. The idea of hierarchical rooflines is that they define multiple ceilings that represent the limiters of a hardware hierarchy. For example, a hierarchical roofline focusing on the memory hierarchy could have ceilings for the throughputs of the L1 cache, L2 cache and device memory. If the achieved performance of a kernel is limited by one of the ceilings of a hierarchical roofline, it can indicate that the corresponding unit of the hierarchy is a potential bottleneck.

![../_images/profiler-report-pages-section-rooflines.png](images/nsight-compute_45.png)
...
When the [Metric Details](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-metric-details) tool window is open, you can click on an achieved value in the chart to see its metric formula in the tool window.

| Zoom In | Zoom Out | Zoom Reset | Pan |
| --- | --- | --- | --- |
| - Click the Zoom In button in the top right corner of the chart. - Click the left mouse button and drag to create a rectangle that bounds the area of interest. - Press the plus (+) key. - Use Ctrl + MouseWheel (Windows and Linux only) | - Click the Zoom Out button in the top right corner of the chart. - Click the right mouse button. - Press the minus (-) key. - Use Ctrl + MouseWheel (Windows and Linux only) | - Click the Zoom Reset button in the top right corner of the chart. - Press the Escape (Esc) key. | - Use Ctrl (Command on Mac) + LeftMouseButton to grab the chart, then move the mouse. - Use the cursor keys. |

##### [Source](https://docs.nvidia.com/nsight-compute/NsightCompute#source)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#source "Permalink to this headline")

Sections such as _Source Counters_ can contain source hot spot tables. These tables indicate the N highest or lowest values of one or more metrics in your kernel source code. Select the location links to navigate directly to this location in the [Source Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-source-page). Hover the mouse over a value to see which metrics contribute to it.

![../_images/profiler-report-pages-details-source-table.png](images/nsight-compute_46.png)

Hot spot tables point out performance problems in your source.

##### [Timelines](https://docs.nvidia.com/nsight-compute/NsightCompute#timelines)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#timelines "Permalink to this headline")

When collecting metrics with [PM sampling](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#pm-sampling), they can be viewed in a _timeline_. The timeline shows metrics selected in the respective section file or on the command line with their labels/names and their values over time.

![../_images/profiler-report-pages-details-timeline.png](images/nsight-compute_47.png)
...
You can hover the mouse over a metric row label to see further information on the metrics in the row. Hovering over a sample on the timeline shows the metric values at that timestamp within the current row. With the [Metric Details](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-metric-details) tool window open, click to select a value on the timeline and show the metric and all its raw timestamps (absolute and relative) correlated values in the tool window.

You can also use the [Metric Details](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-metric-details) tool window to inspect profiler metrics generated during PM sampling. These provide information about the used sampling intervals, buffer sizes, dropped samples and other properties for each collection pass. A detailed list can be found in the [metrics reference](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#metrics-reference).

The timeline has a context menu for further actions regarding copying, zooming or adjusting the viewed data:

> - Metric rows on the timeline can be scaled (y-axis) based on a theoretical HW peak, or based on the maximum profiled value in the workload.
> Select the _Scale to Peak_/_Scale to Max Value_ options in the context menu to switch between the two modes.
> Note that HW peak info may not be available for all rows.
> - When zoomed out, the bar height may be reduced to represent that multiple samples are aggregated into one bar.
> The _Show/Hide Max Bars_ options in the context menu can be used to enable/disable showing the maximum value for each time range across all samples in that range, even when zoomed out.
> - In addition, the _Enable/Disable Context Switch Filter_ option can be used to enable or disable the filtering of the timeline data with [context switch](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#pm-sampling) information, if it is available.
...
> When the context switch filter is disabled, the raw collected sampling data is shown along with a separate row for each pass group’s context switch trace.
> - When the context menu option is not available, the report does not include context switch trace data.
> In this case, the option _Enable/Disable Workload Alignment_ is shown instead if workload execution trace is available.
> When enabled, it aligns all passes based on their first workload execution timestamp.
...
#### [Source Page](https://docs.nvidia.com/nsight-compute/NsightCompute#source-page)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#source-page "Permalink to this headline")
...
![../_images/profiler-report-pages-source.png](images/nsight-compute_48.png)
...
##### [Navigation](https://docs.nvidia.com/nsight-compute/NsightCompute#navigation)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#navigation "Permalink to this headline")

The _View_ dropdown can be used to select different code (correlation) options: SASS, PTX and Source (CUDA-C, Fortran, Python, …).

In side-by-side views, when selecting a line in the left-hand- or right-hand-side, any correlated lines in the opposite view are highlighted. However, when the [Show Single File For Multi-File Sources](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-profile) option is set to _Yes_, the target file or source object must already be selected in the respective view for those correlated lines to be shown.
...
![../_images/profiler-report-pages-correlation-navigation.png](images/nsight-compute_49.png)
...
If a view contains multiple source files or functions, [+] and [-] buttons are shown. These can be used to expand or collapse the view, thereby showing or hiding the file or function content except for its header. These will work for both the views if that group is in the linked state otherwise in an unlinked state it will work for a focused view. If collapsed, all [metrics](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-source-page-metrics) are shown aggregated to provide a quick overview.

![../_images/profiler-report-pages-source-collapse.png](images/nsight-compute_50.png)

Collapsed Source View

The SASS instruction disassembly includes a symbol label column. Symbol labels are displayed alongside instruction addresses, matching the output style of the `nvdisasm` tool.
These labels are interactive hyperlinks and clicking a symbol label will immediately navigate to the corresponding address line within the view.
Labels are shown for branch targets and function calls, making it easy to follow control flow and symbol references directly from the disassembly.
This feature allows you to quickly explore code flow and symbol references.

![../_images/profiler-report-pages-symbol-labels.png](images/nsight-compute_51.png)

Symbol Labels

You can use the search functionality to find information within your data efficiently.
This feature allows users to search within a specific column by selecting it through a click anywhere in the column.
If no column is selected, the search defaults to the _Source_ column.
The functionality supports advanced search expressions, such as regular expressions (regex) and value comparisons (e.g., “>= 5”).
The search control can be opened using the search button on the common control toolbar or by pressing the Ctrl+F keys.
It can be closed using the cross button or by pressing the Esc key.

![../_images/profiler-report-pages-source-search.png](images/nsight-compute_52.png)

Search Control

The SASS view is filtered to only show functions that were executed in the launch. You can toggle the [Show Only Executed Functions](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-profile) option to change this, but performance of this page may be negatively affected for large binaries. It is possible that some SASS instructions are shown as _N/A_. Those instructions are not currently exposed publicly.
...
![../_images/profiler-report-pages-source-navigate-by.png](images/nsight-compute_53.png)

Linked state of “Navigate By” Dropdowns

Only filenames are shown in the view, together with a _File Not Found_ error, if the source files cannot be found in their original location. This can occur, for example, if the report was moved to a different system. Select a filename and click the _Resolve_ button above to specify where this source can be found on the local filesystem. However, the view always shows the source files if the [import source](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#connection-activity-interactive) option was selected during profiling, and the files were available at that time. If a file is found in its original or any source lookup location, but its attributes don’t match, a _File Mismatch_ error is shown. See the [Source Lookup](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-source-lookup) options for changing file lookup behavior.

![../_images/profiler-report-pages-source-resolve.png](images/nsight-compute_54.png)

Resolve Source

If the report was collected using remote profiling, and automatic resolution of remote files is enabled in the [Profile](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-profile) options, NVIDIA Nsight Compute will attempt to load the source from the remote target. If the connection credentials are not yet available in the current NVIDIA Nsight Compute instance, they are prompted in a dialog. Loading from a remote target is currently only available for Linux x86_64 targets and Linux and Windows hosts.

##### [Metrics](https://docs.nvidia.com/nsight-compute/NsightCompute#metrics)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#metrics "Permalink to this headline")

###### [Metrics Correlation](https://docs.nvidia.com/nsight-compute/NsightCompute#metrics-correlation)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#metrics-correlation "Permalink to this headline")
...
![../_images/profiler-report-pages-source-column-chooser.png](images/nsight-compute_55.png)
...
![../_images/profiler-report-pages-fix-column.png](images/nsight-compute_56.png)
...
![../_images/profiler-report-pages-source-heatmap.png](images/nsight-compute_57.png)
...
![../_images/profiler-report-pages-source-rel-abs.png](images/nsight-compute_58.png)

Relative and Absolute Metric Values.

###### [Pre-Defined Source Metrics](https://docs.nvidia.com/nsight-compute/NsightCompute#pre-defined-source-metrics)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#pre-defined-source-metrics "Permalink to this headline")

- **Metric Pipelines**

The [metric pipelines](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#pipelines) this instruction (can) execute in.
Some instructions can execute in one of multiple pipelines, and the decision is dynamic at runtime.
Some instructions execute in both of two pipelines, and the association is static.
Other instructions execute in only a single pipeline.
- **Scoreboard Dependencies**

> See [Instructions & Scoreboards Table](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-source-page-additional-tables) for information on the scoreboard dependencies.
- **Live Registers**
...
Use this to identify source locations that cause the most stalls. The stalls are attributed to the producer location, which is the location of the line that produced the scoreboard.
This may not be the same as the location of the instruction that is stalled, as it may be waiting on a scoreboard produced by another line.
...
The number of samples from the [Statistical Sampler](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#statistical-sampler) at this program location.
- **Warp Stall Sampling (Not-issued Samples)**[^2]

The number of samples from the [Statistical Sampler](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#statistical-sampler) at this program location on cycles the warp scheduler issued no instructions. Note that _(Not Issued)_ samples may be taken on a different profiling pass than _(All)_ samples mentioned above, so their values do not strictly correlate.
...
All _stall_*_ metrics show the information combined in _Warp Stall Sampling_ individually. See [Statistical Sampler](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#statistical-sampler) for their descriptions.
- See the [Customization Guide](https://docs.nvidia.com/nsight-compute/CustomizationGuide/index.html#abstract) on how to add additional metrics for this view and the [Metrics Reference](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#metrics-reference) for further information on available metrics.

###### [Register Dependencies](https://docs.nvidia.com/nsight-compute/NsightCompute#register-dependencies)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#register-dependencies "Permalink to this headline")
...
![../_images/profiler-report-pages-source-register-dependencies.png](images/nsight-compute_59.png)
...
##### [Profiles](https://docs.nvidia.com/nsight-compute/NsightCompute#profiles)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#profiles "Permalink to this headline")

The icon next to the _View_ dropdown can be used to manage _Source View Profiles_.

![../_images/profiler-report-pages-source-profiles-button.png](images/nsight-compute_60.png)
...
![../_images/profiler-report-pages-source-profiles.png](images/nsight-compute_61.png)
...
Simply select the profile from the source view profiles dialog and apply it. You can also make the profile default by clicking on the _star_ button to automatically apply it while opening a report.
...
##### [Additional Tables](https://docs.nvidia.com/nsight-compute/NsightCompute#additional-tables)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#additional-tables "Permalink to this headline")

###### [Instructions & Dependencies Table](https://docs.nvidia.com/nsight-compute/NsightCompute#instructions-dependencies-table)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#instructions-dependencies-table "Permalink to this headline")
...
The breakdown of all instruction categories.
Use this to understand the composition of the selection, and which instruction categories have the most (scoreboard) warp stalls.
- **Input Scoreboard Dependencies**:
The scoreboard-driven data dependencies of the selection.
Data dependencies are predecessor operations that need to complete before code in the selection can proceed.
Use this to understand on which instructions the selection is waiting (stalled) on.

The _Attributed Stalls_ column shows the number of warp stalls from the selection attributed to this dependency.
As a result, dependencies causing more stalls in the selection are sorted higher in the table.
This data is also available in the source views to locate lines causing the most stalls.
- **Output Scoreboard Dependencies**:
The consumers of scoreboards produced by the selection.
A scoreboard producer is an operation producing a scoreboard a consumer is waiting on.
Use this to understand who is waiting (stalled) on the selection.
Note that these lines may wait for other instructions than the current selection, too.
...
The _Dependencies_ tables show in their _Scoreboards_ columns which of the GPU’s six scoreboards are responsible for the dependency.
Use the _Scoreboard Dependencies_ column in the SASS view to see individual scoreboard dependencies between assembly instructions.
...
![../_images/profiler-report-pages-instruction-mix-table.png](images/nsight-compute_62.png)
...
- **Self Instruction Mix**:
The breakdown of all instruction categories.
Use this to understand the composition of the selection.
- **Input Register Dependencies**:
The general purpose register-driven data dependencies of the selection.
Data dependencies are predecessor operations that need to complete before code in the selection can proceed.
Use this to understand which instructions produce/write registers the selection is waiting to consume/read.

The _Attributed Live Registers_ column shows the number of live general-purpose registers attributed to instructions of this category.
This is the maximum number of live registers across all consumers of dependencies from this producer.
This data is also available in the source views to locate lines causing the most live registers.

The _Output Registers_ column shows the number of register dependencies produced by instructions of this category.
This is the sum of all general-purpose registers written by this producer.
This data is also available in the source views to locate lines producing the most dependencies.
- **Output Register Dependencies**:
This table lists all consumers/readers of general-purpose registers produced/written by the selected line.
A register producer is an operation writing a register a consumer is waiting on for reading.
Use this to understand which instructions are waiting on the selection to produce/write registers.
Note that these lines may wait for other instructions than the current selection, too.

![../_images/profiler-report-pages-register-dependencies-table.png](images/nsight-compute_63.png)

Register Dependencies Table

For both scoreboard and register views, the dependencies are grouped by location and instruction category.
If the location does not match the selected line, it is shown as a clickable link. Clicking on the link will navigate to the corresponding line.
If the selection is in a high-level (e.g., CUDA, Python) view, the locations are mapped to the high-level language if possible.
If not possible due to missing correlation information, or if the selection is in the SASS view, the locations are shown in the low-level language.

###### [Inline Functions Table](https://docs.nvidia.com/nsight-compute/NsightCompute#inline-functions-table)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#inline-functions-table "Permalink to this headline")
...
![../_images/profiler-report-pages-inline-functions-table.png](images/nsight-compute_64.png)

Inline Functions Table

###### [Source Markers Table](https://docs.nvidia.com/nsight-compute/NsightCompute#source-markers-table)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#source-markers-table "Permalink to this headline")
...
![../_images/profiler-report-pages-source-markers.png](images/nsight-compute_65.png)
...
![../_images/profiler-report-pages-source-markers-table.png](images/nsight-compute_66.png)

Source Markers Table

###### [Statistics Table](https://docs.nvidia.com/nsight-compute/NsightCompute#statistics-table)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#statistics-table "Permalink to this headline")
...
![../_images/profiler-report-pages-source-statistics.png](images/nsight-compute_67.png)

Statistics Table

##### [Limitations](https://docs.nvidia.com/nsight-compute/NsightCompute#limitations)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#limitations "Permalink to this headline")

**Graph Profiling**

When profiling complete CUDA graphs, instruction-level source metrics are not available.

#### [Context Page](https://docs.nvidia.com/nsight-compute/NsightCompute#context-page)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#context-page "Permalink to this headline")

The _CPU Call Stack_ section of this report page shows the CPU call stack(s) for the executing CPU thread at the time the kernel was launched. For this information to show up in the profiler report, the option to collect CPU call stacks had to be enabled in the [Connection Dialog](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#connection-dialog) or using the corresponding NVIDIA Nsight Compute CLI command line parameter.

![../_images/profiler-report-pages-callstack.png](images/nsight-compute_68.png)

NVIDIA Nsight Compute supports to collect _native_ CPU call stacks as well as call stacks for _Python_ applications.
Either or both types can be selected in the _Activity_ menu of the [Connection Dialog](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#connection-dialog) (via the “CPU Call Stack Types” option),
or using the NVIDIA Nsight Compute [CLI command line parameter](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#command-line-options-launch) *–call-stack-type*.
In case both types are enabled, a dropdown menu will appear to select the desired call stack type.

![../_images/profiler-report-pages-callstack-python.png](images/nsight-compute_69.png)

Note that Python call stack collection requires CPython version 3.9 or later.

The _NVTX State_ section of this report page shows the NVTX context when the kernel was launched. All thread-specific information is with respect to the thread of the kernel’s launch API call. Note that NVTX information is only collected if the profiler is started with NVTX support enabled, either in the [Connection Dialog](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#connection-dialog) or using the NVIDIA Nsight Compute CLI command line parameter.

![../_images/profiler-report-pages-nvtx.png](images/nsight-compute_70.png)

This page has been renamed from “Call Stack / NVTX Page”.

#### [Comments Page](https://docs.nvidia.com/nsight-compute/NsightCompute#comments-page)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#comments-page "Permalink to this headline")

The _Comments_ page aggregates all section comments in a single view and allows the user to edit those comments on any launch instance or section, as well as on the overall report. Comments are persisted with the report. If a section comment is added, the comment icon of the respective section in the [Details Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-details-page) will be highlighted.

#### [Raw Page](https://docs.nvidia.com/nsight-compute/NsightCompute#raw-page)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#raw-page "Permalink to this headline")
...
If a metric has multiple instance values, the number of instances is shown after the standard value. This metric for example has ten instance values: `35.48 {10}`. You can select in the [Profile](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-profile) options dialog that all instance values should be shown individually or inspect the metric result in the [Metric Details](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-metric-details) tool window.

#### [Session Page](https://docs.nvidia.com/nsight-compute/NsightCompute#session-page)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#session-page "Permalink to this headline")
...
### [3.6.3. Metrics and Units](https://docs.nvidia.com/nsight-compute/NsightCompute#metrics-and-units)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#metrics-and-units "Permalink to this headline")
...
### [3.6.4. Filtered Profiler Report](https://docs.nvidia.com/nsight-compute/NsightCompute#filtered-profiler-report)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#filtered-profiler-report "Permalink to this headline")
...
To save the filtered results, you can use the *File > Save Filtered Results As* menu option, which will save the [Filtered Results](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-header-filter-dialog) to a new file.
Alternatively, you can select one or more results on the [Summary Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#summary-page) or [Raw Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#raw-page) and save them to a new file using the *File > Save Selected Results As* menu option or the right-click *Save Result(s)* menu option.

## [3.7. Baselines](https://docs.nvidia.com/nsight-compute/NsightCompute#id8)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#id8 "Permalink to this headline")

NVIDIA Nsight Compute supports diffing collected results across one or multiple reports using Baselines. Each result in any report can be promoted to a baseline. This causes metric values from all results in all reports to show the difference to the baseline. If multiple baselines are selected simultaneously, metric values are compared to the average across all current baselines. Baselines are not stored with a report and are only available as long as the same NVIDIA Nsight Compute instance is open, unless they are saved to a `ncu-bln` file from the [Baselines tool window](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-baselines).

![../_images/baselines.png](images/nsight-compute_71.png)

Profiler report with one baseline

Select _Add Baseline_ to promote the current result in focus to become a baseline. If a baseline is set, most metrics on the [Details Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-details-page), [Raw Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-raw-page) and [Summary Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-summary-page) show two values: the current value of the result in focus, and the corresponding value of the baseline or the percentage of change from the corresponding baseline value. (Note that an infinite percentage gain, _inf%_, may be displayed when the baseline value for the metric is zero, while the focus value is not.)
...
![../_images/baselines-multiple.png](images/nsight-compute_72.png)

Profiler report with multiple baselines

Baseline added for the current result in focus is always shown on the top. However, the actual order of added baselines is shown in the [Baselines tool window](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-baselines).

![../_images/baselines-tool-window.png](images/nsight-compute_73.png)
...
Use the _Clear Baselines_ entry from the dropdown button, the [Profile](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#options-profile) menu, or the corresponding toolbar button to remove all baselines.

Baseline changes can also be made in the [Baselines tool window](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-baselines).

## [3.8. Standalone Source Viewer](https://docs.nvidia.com/nsight-compute/NsightCompute#standalone-source-viewer)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#standalone-source-viewer "Permalink to this headline")

NVIDIA Nsight Compute includes a standalone source viewer for _cubin_ files. This view is identical to the [Source Page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-source-page), except that it won’t include any performance metrics.
...
![../_images/sm-selection-dialog.png](images/nsight-compute_74.png)

SM Selection Dialog

Click Ok button to open [Standalone Source Viewer](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#cubin-viewer).

![../_images/cubin-viewer.png](images/nsight-compute_75.png)

Standalone Source Viewer

## [3.9. Source Comparison](https://docs.nvidia.com/nsight-compute/NsightCompute#source-comparison)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#source-comparison "Permalink to this headline")
...
![../_images/source-comparison-from-header.png](images/nsight-compute_76.png)

Source Comparison Button

![../_images/source-comparison-document.png](images/nsight-compute_77.png)

Source Comparison

Currently only high-level Source (CUDA-C) view and SASS view are supported for comparison.
The source difference heatmap is located on the very left side. Each of its partitions represents a side of the source diff. Clicking on the heatmap will scroll to the respective source line.
...
![../_images/source-comparison-navigation-buttons.png](images/nsight-compute_78.png)
...
![../_images/source-comparison-diff-by-menu.png](images/nsight-compute_79.png)

Source Comparison Diff By Menu

## [3.10. Report Merge Tool](https://docs.nvidia.com/nsight-compute/NsightCompute#report-merge-tool)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#report-merge-tool "Permalink to this headline")

NVIDIA Nsight Compute provides a _Report Merge Tool_ utility that enables users to combine multiple Nsight Compute reports with the `.ncu-rep` extension into a single file.
...
Select the Merge Tool item from Tools in the [Main Menu](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#main-menu).
2. **Launching from the Command Line**

Call the binary in `$NCU_INSTALL_PATH/extras/ReportUtils/ReportMergeTool`.

It accepts a directory as an input, and it will merge all the reports under that directory.
Other features follow exactly the same logic as the UI.

### [3.10.1. File Selection Tab](https://docs.nvidia.com/nsight-compute/NsightCompute#file-selection-tab)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#file-selection-tab "Permalink to this headline")

This tab handles the selection of reports and allows to set the output name and/or path for the resulting merged report.
There are 2 ways of selecting report files to merge.

1. **Selecting from system files**

This option shows a hierarchical view of `.ncu-rep` files in the system, and the containing folders.
Individual reports can be selected together with entire folders.
The _Report Directory_ input changes the root directory of the hierarchical view.

![../_images/mergetool-files.png](images/nsight-compute_80.png)

Selection from System Files
2. **Selecting from currently open reports**

When some reports are already open in Nsight Compute, MergeTool will suggest to merge them first.
To change select from the system files, set _Select from_ combo box item to _System Files_, or use the _Report Directory_ selector.

### [3.10.2. Metric Filters Tab](https://docs.nvidia.com/nsight-compute/NsightCompute#metric-filters-tab)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#metric-filters-tab "Permalink to this headline")

This tab guides the selection and merging process of individual metrics.
Selecting the _Merge Operation_ defines which metrics are merged and kept across all report results.
_Aggregation Operation_ then tells how these metrics are merged, and how the resulting value is calculated from multiple values.
For any other filtering operation metric names can be provided in _Results_ text box, or _Metric Sets/Sections_ can be selected.

![../_images/mergetool-metrics.png](images/nsight-compute_81.png)
...
- **Union** keeps all the metrics that appear at least once across the matching results in all reports.
There can only be one metric set per result, metrics that don’t exist in some reports will appear empty in those cases.
- **Intersection** keeps only the metrics that exist in all matching results across the reports.
This option guarantees that no metrics will have missing values in the output. Note that if reports contain no matching results,
they cannot be merged, which may result in some metrics being empty in the final output.

**Aggregation Operations**

Whenever two or more report results are merged, the metric values are aggregated.
Aggregation operations tell the MergeTool how the metrics are combined together.
As a result, the merged report will contain one of these four options as a metric value.
...
The simplest way to set the metric list is to enter the metrics as a comma-separated list in the _Metrics_ text box.

![../_images/mergetool-metrics-sections.png](images/nsight-compute_82.png)

Metric Sets/Sections

Additionally, metric sets and sections can be used, by loading a view similar to [Metric Selection](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#metric-selection).
Selecting the sets and sections modifies the appearance of the reports by changing the sections that are displayed on the _Details_ page.
The metrics of the reports are not affected.
To remove unwanted sections, select the _Remove Unselected Sections_ checkbox.

### [3.10.3. Result Filters Tab](https://docs.nvidia.com/nsight-compute/NsightCompute#result-filters-tab)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#result-filters-tab "Permalink to this headline")

This tab guides the selection and merging process of individual results.
It allows to select the [Merge Operation](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#mergetool-results-merge-op) to define which results are merged and kept.

For any other filtering operations result names can be provided in the _Results_ text box as a comma-separated list.
If this list is set, only the provided results will appear in the output.

![../_images/mergetool-results.png](images/nsight-compute_83.png)

Result Filter Tab

#### [Result Merge Operations](https://docs.nvidia.com/nsight-compute/NsightCompute#result-merge-operations)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#result-merge-operations "Permalink to this headline")

Result merge operations, similarly to the metric merge operations, tell the MergeTool which results to keep and which results to discard.
Additionally, they can manage the merge process by matching the results from the merged reports.
The matched results are the ones that will be aggregated together into a single result.
The four available options provide control over which results are merged together and how to handle unmatched results.

- **Union** merges only matching results from every report, keeps unmatched results as is.
When multiple iterations of the same kernel appear in a report, they are kept separate and are only merged with the corresponding iteration of this kernel in other reports.

_For example:_ Merging Report1 with results name {kernel_A, kernel_B} and Report2 with {kernel_A, kernel_C} results in {kernel_A<sup>merged</sup> , kernel_B, kernel_C}. i.e.,
...
\(\(\{A, B\} + \{A, C\} = \{A^{merged}\}\)\)
- **Collapse** merges all results sharing the same name.
When multiple iterations of the same kernel appear in a report, merges them together.
...
#### [Merged Report](https://docs.nvidia.com/nsight-compute/NsightCompute#merged-report)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#merged-report "Permalink to this headline")

To be able to track how a merged report was created, each merged report contains a comment with the list of reports that were merged,
the merge operation, the aggregation operation and the sections that were selected.

![../_images/mergetool-comment.png](images/nsight-compute_84.png)

Comment with merge details in the merged report

## [3.11. Clustering Window](https://docs.nvidia.com/nsight-compute/NsightCompute#clustering-window)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#clustering-window "Permalink to this headline")
...
![../_images/clustering-window.png](images/nsight-compute_85.png)

Clustering Window

### [3.11.1. How to Use Clustering](https://docs.nvidia.com/nsight-compute/NsightCompute#how-to-use-clustering)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#how-to-use-clustering "Permalink to this headline")
...
### [3.11.2. Clustering Tree](https://docs.nvidia.com/nsight-compute/NsightCompute#clustering-tree)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#clustering-tree "Permalink to this headline")

![../_images/clustering-tree.png](images/nsight-compute_86.png)
...
### [3.11.3. Similarity Matrix](https://docs.nvidia.com/nsight-compute/NsightCompute#similarity-matrix)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#similarity-matrix "Permalink to this headline")

![../_images/clustering-matrix.png](images/nsight-compute_87.png)

Similarity Matrix

The similarity matrix displays a grid showing how similar each pair of reports is between each other.
This matrix is used to generate the clustering tree - reports with higher similarity scores are grouped closer together in the tree structure.
...
- _Select Reports_
opens a file dialog to choose the `.ncu-rep` files you want to analyze.
- _Run Clustering_
starts the clustering analysis using the selected reports and current parameters.
- _Reset to Default_
restores clustering parameters to their default values.
- _Clear Clusters_
clears the current cluster selection.
- _Suggest Clusters_
automatically recommends optimal clustering parameters based on your data characteristics. The suggestion will cover most of the reports, but outliers may be excluded.
- _Merge Clusters_
creates a merged report from the selected clusters. The merged reports can be processed using the [Report Merge Tool](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#report-merge-tool.html) for further analysis and customization.

## [3.12. Occupancy Calculator](https://docs.nvidia.com/nsight-compute/NsightCompute#occupancy-calculator)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#occupancy-calculator "Permalink to this headline")
...
![../_images/occupancy-calculator-activity.png](images/nsight-compute_88.png)
...
![../_images/occupancy-calculator-from-header.png](images/nsight-compute_89.png)

Details page header

![../_images/occupancy-calculator-from-section.png](images/nsight-compute_90.png)
...
### [3.12.1. Tables](https://docs.nvidia.com/nsight-compute/NsightCompute#tables)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#tables "Permalink to this headline")
...
![../_images/occupancy-calculator-tables.png](images/nsight-compute_91.png)

Tables

### [3.12.2. Utilization](https://docs.nvidia.com/nsight-compute/NsightCompute#utilization)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#utilization "Permalink to this headline")
...
![../_images/occupancy-calculator-utilization.png](images/nsight-compute_92.png)

Utilization

### [3.12.3. Graphs](https://docs.nvidia.com/nsight-compute/NsightCompute#graphs)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#graphs "Permalink to this headline")
...
The “Show As” option allows you to switch between the occupancy in percent and absolute number of warps on Y-axis.
When you hover over the graphs, the nearest data point will be highlighted and its X and Y-axis values will appear in a tooltip.

![../_images/occupancy-calculator-graphs.png](images/nsight-compute_93.png)

Graphs

### [3.12.4. GPU Data](https://docs.nvidia.com/nsight-compute/NsightCompute#gpu-data)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#gpu-data "Permalink to this headline")

The _GPU Data_ shows the properties of all supported devices.

![../_images/occupancy-calculator-gpu-data.png](images/nsight-compute_94.png)

GPU Data

## [3.13. Green Contexts support](https://docs.nvidia.com/nsight-compute/NsightCompute#green-contexts-support)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#green-contexts-support "Permalink to this headline")
...
the [Profiling Guide](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#special-configurations-green-contexts) for an overview about
profiling such applications.

To identify Green Context results quickly, the icon of the _Attributes_ button in the [Profiler Report Header](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-header)
turns into a _leaf_ for Green Context results. Clicking on this button navigates to the _Launch Statistics_ section of the result (if available),
which contains [additional metrics](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#id28) related to the Green Context (e.g., its ID and the number of SMs used).
Some of this information is also available from the tooltip when hovering over the _Attributes_ button.

To distinguish between Green-Contexts _attributable_ and _non-attributable_ metrics when navigating the [Details page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#details-page), _Green Context markers_
may be used. These can be toggled via the _View_ menu of the [Report Header](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#profiler-report-header) and adds a _leaf_ icon
behind _attributable_ metrics in all section headers. When hovering over attributable metrics, the tooltip also shows a hint about the
scaling of such metrics.
Additionally, the [Metrics Details](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#metric-details) tool window shows an _Attribution_ row for metrics of Green Context results.

![../_images/green-contexts-details-page.png](images/nsight-compute_95.png)
...
If a report contains Green Context results, the [Session page](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#session-page) includes an additional _Green Context Resources_ section.
...
assigned to it.
The _Result IDs_ column allows you to navigate to each profile result that uses the corresponding Green Context.

![../_images/green-contexts-session-page.png](images/nsight-compute_96.png)

When profiling Green Context applications interactively, the [Resources](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#resources) tool window also allows to track
the state of all Green Contexts, their resources, as well as their associated streams.

![../_images/green-contexts-resource-tool-window-with-tpc-mask.png](images/nsight-compute_97.png)

## [3.14. Acceleration Structure Viewer](https://docs.nvidia.com/nsight-compute/NsightCompute#acceleration-structure-viewer)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#acceleration-structure-viewer "Permalink to this headline")
...
The _Acceleration Structure Viewer_ is opened through a button in the [Resources](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#tool-window-resources) window. The button will only be available when the currently viewed resource is _OptiX: TraversableHandles_. It opens the currently selected handle.

![../_images/as-viewer-open-button.png](images/nsight-compute_98.png)
...
![../_images/as-viewer.png](images/nsight-compute_99.png)
...
### [3.14.1. Navigation](https://docs.nvidia.com/nsight-compute/NsightCompute#as-viewer-nav)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#as-viewer-nav "Permalink to this headline")
...
![../_images/as-viewer-cam.png](images/nsight-compute_100.png)

### [3.14.2. Filtering and Highlighting](https://docs.nvidia.com/nsight-compute/NsightCompute#filtering-and-highlighting)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#filtering-and-highlighting "Permalink to this headline")
...
![../_images/as-viewer-display-filter.png](images/nsight-compute_101.png)
...
![../_images/as-viewer-property-filter.png](images/nsight-compute_102.png)

### [3.14.3. Rendering Options](https://docs.nvidia.com/nsight-compute/NsightCompute#rendering-options)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#rendering-options "Permalink to this headline")
...
![../_images/as-viewer-rendering-options.png](images/nsight-compute_103.png)

### [3.14.4. Exporting](https://docs.nvidia.com/nsight-compute/NsightCompute#exporting)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#exporting "Permalink to this headline")
...
## [3.15. Options](https://docs.nvidia.com/nsight-compute/NsightCompute#options)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#options "Permalink to this headline")
...
![../_images/options-profile.png](images/nsight-compute_104.png)

Profile options

### [3.15.1. Profile](https://docs.nvidia.com/nsight-compute/NsightCompute#profile)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#profile "Permalink to this headline")

#### [Report Sections](https://docs.nvidia.com/nsight-compute/NsightCompute#report-sections)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#report-sections "Permalink to this headline")
...
#### [Report Rules](https://docs.nvidia.com/nsight-compute/NsightCompute#report-rules)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#report-rules "Permalink to this headline")
...
#### [Report UI](https://docs.nvidia.com/nsight-compute/NsightCompute#report-ui)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#report-ui "Permalink to this headline")

Customize the user interface behavior for report viewing and navigation.

| Option Name | Description | Values |
| --- | --- | --- |
| Default Report Page | The report page to show when a report is generated or opened. _Auto_ lets the tool decide the best page to show when opening a report. | - Summary (Default) - Details - Source - Context - Comments - Raw - Session - Auto |
| Function Name Mode | Determines how function/kernel names are shown. | - Auto (Default) - Demangled - Function - Mangled |
| NVTX Rename Mode | Determines how NVTX information is used for renaming. Range replay results are always renamed when possible. | - None - Kernel - Resources (Default) - All |
...
#### [Report Baselines](https://docs.nvidia.com/nsight-compute/NsightCompute#report-baselines)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#report-baselines "Permalink to this headline")
...
#### [Report Metrics](https://docs.nvidia.com/nsight-compute/NsightCompute#report-metrics)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#report-metrics "Permalink to this headline")
...
#### [Report Summary Page](https://docs.nvidia.com/nsight-compute/NsightCompute#report-summary-page)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#report-summary-page "Permalink to this headline")
...
#### [Report Details Page](https://docs.nvidia.com/nsight-compute/NsightCompute#report-details-page)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#report-details-page "Permalink to this headline")
...
#### [Report Source Page](https://docs.nvidia.com/nsight-compute/NsightCompute#report-source-page)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#report-source-page "Permalink to this headline")
...
#### [API Stream View](https://docs.nvidia.com/nsight-compute/NsightCompute#api-stream-view)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#api-stream-view "Permalink to this headline")
...
### [3.15.2. Fonts and Colors](https://docs.nvidia.com/nsight-compute/NsightCompute#fonts-and-colors)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#fonts-and-colors "Permalink to this headline")

#### [Fonts](https://docs.nvidia.com/nsight-compute/NsightCompute#fonts)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#fonts "Permalink to this headline")

Configure the fonts used throughout the application interface. General fonts affect all UI elements except code,
while code fonts are specifically used for source code display and similar content.
...
#### [Colors](https://docs.nvidia.com/nsight-compute/NsightCompute#colors)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#colors "Permalink to this headline")

Customize the visual appearance of the application through theme selection and color scale preferences.
These settings affect the overall look and feel of the interface and how data is represented visually.

| Option Name | Description | Values |
| --- | --- | --- |
| Color Theme | The currently selected application color theme. | - Dark (Default) - Light |
| Default Color Scale | Default color map used to represent a qualitative scale of values. | - Flammenmeer - Viridis - Inferno - Magma - Plasma - Jet (Default) |

### [3.15.3. Environment](https://docs.nvidia.com/nsight-compute/NsightCompute#environment)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#environment "Permalink to this headline")

#### [Visual Experience](https://docs.nvidia.com/nsight-compute/NsightCompute#visual-experience)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#visual-experience "Permalink to this headline")
...
#### [Windowing](https://docs.nvidia.com/nsight-compute/NsightCompute#windowing)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#windowing "Permalink to this headline")
...
#### [Documents Folder](https://docs.nvidia.com/nsight-compute/NsightCompute#documents-folder)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#documents-folder "Permalink to this headline")
...
#### [Startup Behavior](https://docs.nvidia.com/nsight-compute/NsightCompute#startup-behavior)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#startup-behavior "Permalink to this headline")

Define how NVIDIA Nsight Compute behaves when launched.

| Option Name | Description | Values |
| --- | --- | --- |
| At Startup | What to do when the host is launched. | - Show Welcome Page (Default) - Show Quick Launch Dialog - Load Last Project - Show Empty Environment |

#### [Updates](https://docs.nvidia.com/nsight-compute/NsightCompute#updates)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#updates "Permalink to this headline")
...
### [3.15.4. Connection](https://docs.nvidia.com/nsight-compute/NsightCompute#connection)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#connection "Permalink to this headline")

Connection properties are grouped into _Target Connection Options_ and _Host Connection Properties_.

#### [Target Connection Properties](https://docs.nvidia.com/nsight-compute/NsightCompute#target-connection-properties)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#target-connection-properties "Permalink to this headline")
...
#### [Host Connection Properties](https://docs.nvidia.com/nsight-compute/NsightCompute#host-connection-properties)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#host-connection-properties "Permalink to this headline")
...
#### [SSH ProxyJump Connection Properties](https://docs.nvidia.com/nsight-compute/NsightCompute#ssh-proxyjump-connection-properties)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#ssh-proxyjump-connection-properties "Permalink to this headline")
...
### [3.15.5. Timeline](https://docs.nvidia.com/nsight-compute/NsightCompute#timeline)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#timeline "Permalink to this headline")

#### [Basic Settings](https://docs.nvidia.com/nsight-compute/NsightCompute#basic-settings)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#basic-settings "Permalink to this headline")
...
### [3.15.6. Source Lookup](https://docs.nvidia.com/nsight-compute/NsightCompute#source-lookup)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#source-lookup "Permalink to this headline")

The Source Lookup options control how NVIDIA Nsight Compute locates and validates CUDA source files when displaying source code in the Source page.
These settings are particularly important for debugging and profiling sessions where source files may have moved from their original compilation
locations or when working with different development environments. The options help ensure accurate source code resolution and provide
flexibility in handling file location mismatches.
...
### [3.15.7. Send Feedback…](https://docs.nvidia.com/nsight-compute/NsightCompute#send-feedback)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#send-feedback "Permalink to this headline")
...
## [3.16. Projects](https://docs.nvidia.com/nsight-compute/NsightCompute#projects)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#projects "Permalink to this headline")
...
When no custom project is current, a _default project_ is used to store e.g. the current [Start Activity Dialog](https://docs.nvidia.com/nsight-compute/NsightCompute/index.html#connection-dialog) entries. To remove all information from the default project, you must close NVIDIA Nsight Compute and then delete the file from disk.
...
### [3.16.1. Project Dialogs](https://docs.nvidia.com/nsight-compute/NsightCompute#project-dialogs)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#project-dialogs "Permalink to this headline")
...
### [3.16.2. Project Explorer](https://docs.nvidia.com/nsight-compute/NsightCompute#project-explorer)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#project-explorer "Permalink to this headline")
...
![../_images/projects-explorer.png](images/nsight-compute_105.png)

Project Explorer

## [3.17. Visual Studio Integration Guide](https://docs.nvidia.com/nsight-compute/NsightCompute#visual-studio-integration-guide)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#visual-studio-integration-guide "Permalink to this headline")
...
### [3.17.1. Visual Studio Integration Overview](https://docs.nvidia.com/nsight-compute/NsightCompute#visual-studio-integration-overview)[](https://docs.nvidia.com/nsight-compute/NsightCompute/#visual-studio-integration-overview "Permalink to this headline")
