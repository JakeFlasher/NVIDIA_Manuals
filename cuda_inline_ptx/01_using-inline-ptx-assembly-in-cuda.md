---
title: "1. Using Inline PTX Assembly in CUDA"
section: "1"
source: "https://docs.nvidia.com/cuda/inline-ptx-assembly/#using-inline-ptx-assembly-in-cuda"
---

# [1. Using Inline PTX Assembly in CUDA](https://docs.nvidia.com/cuda/inline-ptx-assembly#using-inline-ptx-assembly-in-cuda)[](https://docs.nvidia.com/cuda/inline-ptx-assembly/#using-inline-ptx-assembly-in-cuda "Permalink to this headline")

The NVIDIA<sup>®</sup> CUDA<sup>®</sup> programming environment provides a parallel thread execution (PTX) instruction set architecture (ISA) for using the GPU as a data-parallel computing device. For more information on the PTX ISA, refer to the latest version of the [PTX ISA reference document](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html).

This application note describes how to inline PTX assembly language statements into CUDA code.

## [1.1. Assembler (ASM) Statements](https://docs.nvidia.com/cuda/inline-ptx-assembly#assembler-asm-statements)[](https://docs.nvidia.com/cuda/inline-ptx-assembly/#assembler-asm-statements "Permalink to this headline")
...
### [1.1.1. Parameters](https://docs.nvidia.com/cuda/inline-ptx-assembly#parameters)[](https://docs.nvidia.com/cuda/inline-ptx-assembly/#parameters "Permalink to this headline")
...
### [1.1.2. Constraints](https://docs.nvidia.com/cuda/inline-ptx-assembly#constraints)[](https://docs.nvidia.com/cuda/inline-ptx-assembly/#constraints "Permalink to this headline")
...
Note that the constraint `"q"` is only supported on platforms that support `__int128`.
...
The constraint `"C"` can be used for operand of type ‘array of const char’, where the array contents are known at compile time.
...
for the `"C"` constraint:

```c++
'C'(constant-expression)
```

The _constant-expression_ is evaluated during compilation and shall generate the address of a variable `V`, where:

- `V` has _static storage duration_.
- `V` has type ‘array of const char’.
- `V` is _constant-initialized_.
- If `V` is a static class member, then `V`’s _initializing declaration_ is the declaration within the class.

During translation, the compiler will replace a reference to the operand within the _Assembler Template_ with the contents of `V`’s initializer, except for the last trailing zero.
No constraint modifiers are allowed for this constraint. This constraint can only be used in device code.

(terms in _italics_ are C++ standard terms and/or terms from the GNU inline asm specification).

Here’s an example of the use of `C` constraint to generate different PTX instruction modes based on compile time computation:
...
There is no constraint letter for 8-bit wide PTX registers. PTX instructions types accepting 8-bit wide types [permit operands to be wider than the instruction-type size](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#restricted-use-of-sub-word-sizes). Example:
...
## [1.2. Pitfalls](https://docs.nvidia.com/cuda/inline-ptx-assembly#pitfalls)[](https://docs.nvidia.com/cuda/inline-ptx-assembly/#pitfalls "Permalink to this headline")

Although `asm()` statements are very flexible and powerful, you may encounter some pitfalls—these are listed in this section.

### [1.2.1. Namespace Conflicts](https://docs.nvidia.com/cuda/inline-ptx-assembly#namespace-conflicts)[](https://docs.nvidia.com/cuda/inline-ptx-assembly/#namespace-conflicts "Permalink to this headline")
...
### [1.2.2. Memory Space Conflicts](https://docs.nvidia.com/cuda/inline-ptx-assembly#memory-space-conflicts)[](https://docs.nvidia.com/cuda/inline-ptx-assembly/#memory-space-conflicts "Permalink to this headline")
...
### [1.2.3. Incorrect Optimization](https://docs.nvidia.com/cuda/inline-ptx-assembly#incorrect-optimization)[](https://docs.nvidia.com/cuda/inline-ptx-assembly/#incorrect-optimization "Permalink to this headline")
...
### [1.2.4. Incorrect PTX](https://docs.nvidia.com/cuda/inline-ptx-assembly#incorrect-ptx)[](https://docs.nvidia.com/cuda/inline-ptx-assembly/#incorrect-ptx "Permalink to this headline")
...
## [1.3. Error Checking](https://docs.nvidia.com/cuda/inline-ptx-assembly#error-checking)[](https://docs.nvidia.com/cuda/inline-ptx-assembly/#error-checking "Permalink to this headline")
