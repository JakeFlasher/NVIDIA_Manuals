---
title: "17. Preparing for Deployment"
section: "17"
source: "https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#preparing-for-deployment"
---

# [17. Preparing for Deployment](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#preparing-for-deployment)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#preparing-for-deployment "Permalink to this headline")

## [17.1. Testing for CUDA Availability](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#testing-for-cuda-availability)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#testing-for-cuda-availability "Permalink to this headline")
...
The `cudaGetDeviceProperties()` function reports various features of the available devices, including the [CUDA Compute Capability](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#cuda-compute-capability) of the device (see also the Compute Capabilities section of the CUDA C++ Programming Guide). See [Version Management](https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART____VERSION.html#group__CUDART____VERSION) for details on how to query the available CUDA software API versions.

## [17.2. Error Handling](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#error-handling)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#error-handling "Permalink to this headline")
...
## [17.3. Building for Maximum Compatibility](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#building-for-maximum-compatibility)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#building-for-maximum-compatibility "Permalink to this headline")

Each generation of CUDA-capable device has an associated _compute capability_ version that indicates the feature set supported by the device (see [CUDA Compute Capability](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#cuda-compute-capability)). One or more compute capability versions can be specified to the nvcc compiler while building a file; compiling for the native compute capability for the target GPU(s) of the application is important to ensure that application kernels achieve the best possible performance and are able to use the features that are available on a given generation of GPU.
...
appropriate binary at runtime according to the compute capability of the present device. If an appropriate native binary (_cubin_)
is not available, but the intermediate _PTX_ code (which targets an abstract virtual instruction set and is used for forward-compatibility)
is available, then the kernel will be compiled _Just In Time_ (JIT) (see [Compiler JIT Cache Management Tools](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#compiler-jit-cache-management))
...
## [17.4. Distributing the CUDA Runtime and Libraries](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#distributing-the-cuda-runtime-and-libraries)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#distributing-the-cuda-runtime-and-libraries "Permalink to this headline")

CUDA applications are built against the CUDA Runtime library, which handles device, memory, and kernel management. Unlike the CUDA Driver, the CUDA Runtime guarantees neither forward nor backward binary compatibility across versions. It is therefore best to [redistribute](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#redistribution) the CUDA Runtime library with the application when using dynamic linking or else to statically link against the CUDA Runtime. This will ensure that the executable will be able to run even if the user does not have the same CUDA Toolkit installed that the application was built against.
...
the `--cudart=shared` flag to the link command line; otherwise the [statically-linked CUDA Runtime library](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#statically-linked-cuda-runtime) is used by default.

After the application is dynamically linked against the CUDA Runtime, this version of the runtime library should be [bundled with](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#redistribution) the application. It can be copied into the same directory as the application executable or into a subdirectory of that installation path.
...
the [dynamically-linked version of the CUDA Runtime library](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#dynamically-linked-cuda-runtime), these libraries should
be [bundled with](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#redistribution) the application executable when distributing that application.

### [17.4.1. CUDA Toolkit Library Redistribution](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#cuda-toolkit-library-redistribution)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#cuda-toolkit-library-redistribution "Permalink to this headline")

The CUDA Toolkit’s End-User License Agreement (EULA) allows for redistribution of many of the CUDA libraries under certain terms and conditions. This allows applications that depend on these libraries [to redistribute the exact versions](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#redistribution-which-files) of the libraries against which they were built and tested, thereby avoiding any trouble for end users who might have a different version of the CUDA Toolkit (or perhaps none at all) installed on their machines. Please refer to the EULA for details.
...
#### [17.4.1.1. Which Files to Redistribute](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#which-files-to-redistribute)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#which-files-to-redistribute "Permalink to this headline")
...
Furthermore, this file should be installed into the `@rpath` of the application; see [Where to Install Redistributed CUDA Libraries](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#redistribution-where-to-install).
...
#### [17.4.1.2. Where to Install Redistributed CUDA Libraries](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#where-to-install-redistributed-cuda-libraries)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#where-to-install-redistributed-cuda-libraries "Permalink to this headline")
