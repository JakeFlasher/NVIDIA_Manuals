...
section: "10"
source: "https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#memory-optimizations"
---

# [10. Memory Optimizations](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#memory-optimizations)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#memory-optimizations "Permalink to this headline")
...
## [10.1. Data Transfer Between Host and Device](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#data-transfer-between-host-and-device)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#data-transfer-between-host-and-device "Permalink to this headline")
...
Finally, higher bandwidth between the host and the device is achieved when using _page-locked_ (or _pinned_) memory, as discussed in the CUDA C++ Programming Guide and the [Pinned Memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#pinned-memory) section of this document.

### [10.1.1. Pinned Memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#pinned-memory)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#pinned-memory "Permalink to this headline")
...
[Asynchronous and Overlapping Transfers with Computation](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#asynchronous-transfers-and-overlapping-transfers-with-computation) illustrates the basic technique.
...
[Staged concurrent copy and execute](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#staged-concurrent-copy-and-execute) shows how the transfer and kernel execution can be broken up into nStreams stages. This approach permits some overlapping of the data transfer and execution.
...
(In [Staged concurrent copy and execute](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#staged-concurrent-copy-and-execute), it is assumed that _N_ is evenly divisible by `nThreads*nStreams`.) Because execution within
...
in [Figure 1](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#timeline-comparison-for-copy-and-kernel-execution-figure), and `nStreams` is equal to 4
for [Staged concurrent copy and execute](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#staged-concurrent-copy-and-execute) in the bottom half of the figure.

![Timeline comparison for copy and kernel execution](images/memory-optimizations_1.png)
...
### [10.1.3. Zero Copy](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#zero-copy)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#zero-copy "Permalink to this headline")
...
The host code in [Zero-copy host code](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#zero-copy-host-code) shows how zero copy is typically set up.
...
in [Zero-copy host code](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#zero-copy-host-code), `kernel()` can reference the mapped pinned host memory using the pointer `a_map` in exactly the
...
### [10.1.4. Unified Virtual Addressing](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#unified-virtual-addressing)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#unified-virtual-addressing "Permalink to this headline")

Devices of [compute capability](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#cuda-compute-capability) 2.0 and later support a special addressing mode called _Unified Virtual Addressing_ (UVA) on 64-bit Linux and Windows. With UVA, the host memory and the device memories of all installed supported devices share a single virtual address space.
...
## [10.2. Device Memory Spaces](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#device-memory-spaces)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#device-memory-spaces "Permalink to this headline")
...
memory spaces include global, local, shared, texture, and registers, as shown in [Figure 2](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#memory-spaces-cuda-device-figure).

![Memory spaces on a CUDA device](images/memory-optimizations_2.png)

Figure 2 Memory spaces on a CUDA device

Of these different memory spaces, global memory is the most plentiful; see Features and Technical Specifications of the CUDA C++ Programming Guide for the amounts of memory available in each memory space at each [compute capability](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#cuda-compute-capability) level. Global, local, and texture memory have the greatest access latency, followed by constant memory, shared memory, and the register file.

The various principal traits of the memory types are shown in [Table 1](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#salient-features-device-memory-table).
...
### [10.2.1. Coalesced Access to Global Memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#coalesced-access-to-global-memory)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#coalesced-access-to-global-memory "Permalink to this headline")
...
#### [10.2.1.1. A Simple Access Pattern](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#a-simple-access-pattern)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#a-simple-access-pattern "Permalink to this headline")
...
![Coalesced access](images/memory-optimizations_3.png)
...
have been performed by a device with [compute capability](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#cuda-compute-capability) 6.0 or higher.

#### [10.2.1.2. A Sequential but Misaligned Access Pattern](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#a-sequential-but-misaligned-access-pattern)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#a-sequential-but-misaligned-access-pattern "Permalink to this headline")
...
will be requested, as shown in [Figure 4](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#misaligned-sequential-addresses-fall-5-32-byte-l2-cache-seqments).

![Misaligned sequential addresses that fall within five 32-byte segments](images/memory-optimizations_4.png)
...
#### [10.2.1.3. Effects of Misaligned Accesses](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#effects-of-misaligned-accesses)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#effects-of-misaligned-accesses "Permalink to this headline")
...
in [A copy kernel that illustrates misaligned accesses](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#a-copy-kernel-that-illustrates-misaligned-accesses).
...
In [A copy kernel that illustrates misaligned accesses](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#a-copy-kernel-that-illustrates-misaligned-accesses), data is copied from the input array `idata` to the output array, both
...
(for example, [Figure 4](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#misaligned-sequential-addresses-fall-5-32-byte-l2-cache-seqments) corresponds to this misalignments).
The effective bandwidth for the copy with various offsets on an NVIDIA Tesla V100 ([compute capability](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#cuda-compute-capability) 7.0)
is shown in [Figure 5](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#performance-offsetcopy-kernel-figure).

![Performance of offsetCopy kernel](images/memory-optimizations_5.png)

Figure 5 Performance of offsetCopy kernel

For the NVIDIA Tesla V100, global memory accesses with no offset or with offsets that are multiples of 8 words result in four 32-byte transactions. The achieved bandwidth is approximately 790 GB/s. Otherwise, five 32-byte segments are loaded per warp, and we would expect approximately 4/5<sup>th</sup> of the memory throughput achieved with no offsets.

In this particular example, the offset memory throughput achieved is, however, approximately 9/10<sup>th</sup>, because adjacent warps reuse the cache lines their neighbors fetched. So while the impact is still evident it is not as large as we might have expected. It would have been more so if adjacent warps had not exhibited such a high degree of reuse of the over-fetched cache lines.

#### [10.2.1.4. Strided Accesses](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#strided-accesses)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#strided-accesses "Permalink to this headline")
...
[Figure 6](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#adjacent-threads-accessing-memory-with-stride-of-2-figure) illustrates such a situation; in this case, threads within a warp access words in memory with a stride of 2. This action leads to a load of eight L2 cache segments per warp on the Tesla V100 (compute capability 7.0).

![Adjacent threads accessing memory with a stride of 2](images/memory-optimizations_6.png)
...
for the 32 threads in a warp, as indicated in [Figure 7](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#performance-of-stridecopy-kernel-figure).

![Performance of strideCopy kernel](images/memory-optimizations_7.png)

Figure 7 Performance of strideCopy kernel

As illustrated in [Figure 7](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#performance-of-stridecopy-kernel-figure), non-unit-stride global memory accesses should be avoided whenever possible. One method for doing so utilizes shared memory, which is discussed in the next section.

### [10.2.2. L2 Cache](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#l2-cache)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#l2-cache "Permalink to this headline")
...
#### [10.2.2.1. L2 Cache Access Window](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#l2-cache-access-window)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#l2-cache-access-window "Permalink to this headline")
...
#### [10.2.2.2. Tuning the Access Window Hit-Ratio](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#tuning-the-access-window-hit-ratio)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#tuning-the-access-window-hit-ratio "Permalink to this headline")
...
![Mapping Persistent data accesses to set-aside L2 in sliding window experiment](images/memory-optimizations_8.png)
...
![The performance of the sliding-window benchmark with fixed hit-ratio of 1.0](images/memory-optimizations_9.png)
...
![The performance of the sliding-window benchmark with tuned hit-ratio](images/memory-optimizations_10.png)

Figure 10 The performance of the sliding-window benchmark with tuned hit-ratio

### [10.2.3. Shared Memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#shared-memory)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#shared-memory "Permalink to this headline")
...
#### [10.2.3.1. Shared Memory and Memory Banks](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#shared-memory-and-memory-banks)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#shared-memory-and-memory-banks "Permalink to this headline")
...
#### [10.2.3.2. Shared Memory in Matrix Multiplication (C=AB)](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#shared-memory-in-matrix-multiplication-c-ab)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#shared-memory-in-matrix-multiplication-c-ab "Permalink to this headline")
...
A natural decomposition of the problem is to use a block and tile size of wxw threads. Therefore, in terms of wxw tiles, A is a column matrix, B is a row matrix, and C is their outer product; see [Figure 11](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#shared-memory-in-matrix-multiplication-c-ab-block-column-matrix-a-multiplied-block-row-matrix-b-product-matrix-c). A grid of N/w by M/w blocks is launched, where each thread block calculates the elements of a different tile in C from a single tile of A and a single tile of B.

![Block-column matrix multiplied by block-row matrix. Block-column matrix (A) multiplied by block-row matrix (B) with resulting product matrix (C).](images/memory-optimizations_11.png)
...
To do this, the `simpleMultiply` kernel ([Unoptimized matrix multiplication](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#unoptimized-matrix-multiplication-example)) calculates the output elements of a tile of matrix C.
...
in [Using shared memory to improve the global memory load efficiency in matrix multiplication](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#using-shared-memory-to-improve-the-global-memory-load-efficiency-in-matrix-multiplication).
...
In [Using shared memory to improve the global memory load efficiency in matrix multiplication](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#using-shared-memory-to-improve-the-global-memory-load-efficiency-in-matrix-multiplication), each element in a tile of A is read from global memory only once, in a fully coalesced fashion (with no wasted bandwidth), to shared memory. Within each iteration of the `for` loop, a value in shared memory is broadcast to all threads in a warp. Instead of a `__syncthreads()`synchronization barrier call, a `__syncwarp()` is sufficient after reading the tile of A into shared memory because only threads within the warp that write the data into shared memory read this data. This kernel has an effective bandwidth of 144.4 GB/s on an NVIDIA Tesla V100. This illustrates the use of the shared memory as a _user-managed cache_ when the hardware L1 cache eviction policy does not match up well with the needs of the application or when L1 cache is not used for reads from global memory.

A further improvement can be made to how [Using shared memory to improve the global memory load efficiency in matrix multiplication](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#using-shared-memory-to-improve-the-global-memory-load-efficiency-in-matrix-multiplication) deals with matrix B. In calculating each of the rows of a tile of matrix C, the entire tile of B is read. The repeated reading of the B tile can be eliminated by reading it into shared memory once ([Improvement by reading additional data into shared memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#improvement-by-reading-additional-data-into-shared-memory)).
...
Note that in [Improvement by reading additional data into shared memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#improvement-by-reading-additional-data-into-shared-memory), a `__syncthreads()` call is required after reading the B tile because a warp reads data from shared memory that were written to shared memory by different warps. The effective bandwidth of this routine is 195.5 GB/s on an NVIDIA Tesla V100. Note that the performance improvement is not due to improved coalescing in either case, but to avoiding redundant transfers from global memory.

The results of the various optimizations are summarized in [Table 2](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#performance-improvements-optimizing-c-ab-matrix-table).
...
#### [10.2.3.3. Shared Memory in Matrix Multiplication (C=AAT)](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#shared-memory-in-matrix-multiplication-c-aat)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#shared-memory-in-matrix-multiplication-c-aat "Permalink to this headline")

A variant of the previous matrix multiplication can be used to illustrate how strided accesses to global memory, as well as shared memory bank conflicts, are handled. This variant simply uses the transpose of A in place of B, so C = AA<sup>T</sup>.

A simple implementation for C = AA<sup>T</sup> is shown in [Unoptimized handling of strided accesses to global memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#unoptimized-handling-of-strided-accesses-to-global-memory).
...
shown in [An optimized handling of strided accesses using coalesced reads from global memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#an-optimized-handling-of-strided-accesses-using-coalesced-reads-from-global-memory).
...
[An optimized handling of strided accesses using coalesced reads from global memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#an-optimized-handling-of-strided-accesses-using-coalesced-reads-from-global-memory) uses the shared `transposedTile` to avoid uncoalesced accesses in the second term in the dot product and the shared `aTile` technique from the previous example to avoid uncoalesced accesses in the first term. The effective bandwidth of this kernel is 140.2 GB/s on an NVIDIA Tesla V100.These results are lower than those obtained by the final kernel for C = AB. The cause of the difference is shared memory bank conflicts.
...
The results of these optimizations are summarized in [Table 3](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#performance-inmprovements-optimizing-c-aa-matrix-multiplication-table).
...
These results should be compared with those in [Table 2](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#performance-improvements-optimizing-c-ab-matrix-table). As can be seen from these tables, judicious use of shared memory can dramatically improve performance.
...
#### [10.2.3.4. Asynchronous Copy from Global Memory to Shared Memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#asynchronous-copy-from-global-memory-to-shared-memory)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#asynchronous-copy-from-global-memory-to-shared-memory "Permalink to this headline")
...
### [10.2.4. Local Memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#local-memory)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#local-memory "Permalink to this headline")
...
### [10.2.5. Texture Memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#texture-memory)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#texture-memory "Permalink to this headline")
...
#### [10.2.5.1. Additional Texture Capabilities](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#additional-texture-capabilities)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#additional-texture-capabilities "Permalink to this headline")

If textures are fetched using `tex1D()`,`tex2D()`, or `tex3D()` rather than `tex1Dfetch()`, the hardware provides other capabilities that might be useful for some applications such as image processing, as shown in [Table 4](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#useful-features-for-tex1d-tex2d-tex3d-fetches-table).
...
### [10.2.6. Constant Memory](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#constant-memory)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#constant-memory "Permalink to this headline")
...
### [10.2.7. Registers](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#registers)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#registers "Permalink to this headline")
...
#### [10.2.7.1. Register Pressure](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#register-pressure)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#register-pressure "Permalink to this headline")
...
## [10.3. Allocation](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#allocation)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#allocation "Permalink to this headline")
...
## [10.4. NUMA Best Practices](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide#numa-best-practices)[](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#numa-best-practices "Permalink to this headline")
