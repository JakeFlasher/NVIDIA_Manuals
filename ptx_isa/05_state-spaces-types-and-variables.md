---
title: "5. State Spaces, Types, and Variables"
section: "5"
source: "https://docs.nvidia.com/cuda/parallel-thread-execution/#state-spaces-types-and-variables"
---

# [5. State Spaces, Types, and Variables](https://docs.nvidia.com/cuda/parallel-thread-execution#state-spaces-types-and-variables)[](https://docs.nvidia.com/cuda/parallel-thread-execution/#state-spaces-types-and-variables "Permalink to this headline")
...
| `.shared` | Yes | No | R/W | per-cluster<sup>5</sup> |
| `.tex` | No<sup>4</sup> | Yes, via driver | RO | Context |
| **Notes:**   <sup>1</sup> Variables in `.const` and `.global` state spaces are initialized to zero by default.   <sup>2</sup> Accessible only via the `ld.param{::entry}` instruction. Address may be taken via `mov` instruction.   <sup>3</sup> Accessible via `ld.param{::func}` and `st.param{::func}` instructions. Device function input and return parameters may have their address taken via `mov`; the parameter is then located on the stack frame and its address is in the `.local` state space.   <sup>4</sup> Accessible only via the `tex` instruction.   <sup>5</sup> Visible to the owning CTA and other active CTAs in the cluster. |  |  |  |  |
...
### [5.1.2. Special Register State Space](https://docs.nvidia.com/cuda/parallel-thread-execution#special-register-state-space)[](https://docs.nvidia.com/cuda/parallel-thread-execution/#special-register-state-space "Permalink to this headline")
...
eleven 64 KB banks, and banks were specified using the `.const[bank]` modifier, where _bank_
...
were used to declare _incomplete_ constant arrays (as in C, for example), where the size is not
...
When compiling to use the _Application Binary Interface (ABI)_, `.local` state-space variables
...
> stack locations based on the function calling conventions of the _Application Binary Interface
> (ABI)_. Therefore, PTX code should make no assumptions about the relative locations or ordering
...
type kernel parameter. The alignment value _N_, if present, must be a power of two. If no state
...
## [5.2. Types](https://docs.nvidia.com/cuda/parallel-thread-execution#types)[](https://docs.nvidia.com/cuda/parallel-thread-execution/#types "Permalink to this headline")
...
packed value is considered as a value of a _packed data type_. In this section we describe the packed
...
PTX includes built-in _opaque_ types for defining texture, sampler, and surface descriptor
...
_opaque_. The use of these opaque types is limited to:
...
textures and samplers, PTX has two modes of operation. In the _unified mode,_ texture and sampler
information is accessed through a single `.texref` handle. In the _independent mode_, texture and
...
## [5.4. Variables](https://docs.nvidia.com/cuda/parallel-thread-execution#variables)[](https://docs.nvidia.com/cuda/parallel-thread-execution/#variables "Permalink to this headline")
...
### [5.4.3. Array Declarations](https://docs.nvidia.com/cuda/parallel-thread-execution#array-declarations)[](https://docs.nvidia.com/cuda/parallel-thread-execution/#array-declarations "Permalink to this headline")
...
_kernel_ above, 19*19 = 361 halfwords are reserved, for a total of 722 bytes.
...
statically initialize a pointer to a variable. Initializers may also contain _var+offset_
expressions, where _offset_ is a byte offset added to the address of _var_. Only variables in
...
kernels from GPU. See the _CUDA Dynamic Parallelism Programming Guide_ for details.
...
declaration. Alignment is specified using an optional `.align` _byte-count_ specifier immediately
...
### [5.4.7. Variable Attributes](https://docs.nvidia.com/cuda/parallel-thread-execution#variable-attributes)[](https://docs.nvidia.com/cuda/parallel-thread-execution/#variable-attributes "Permalink to this headline")
...
[Variable and Function Attribute Directive: .attribute](https://docs.nvidia.com/cuda/parallel-thread-execution/#variable-and-function-attribute-directive-attribute) describes the `.attribute`
directive.
...
**`.managed`**
  ...
  directly. This attribute can only be used with variables in .global state space. See the _CUDA
  UVM-Lite Programming Guide_ for details.
...
#### [5.5.1.1. Sub-byte Types](https://docs.nvidia.com/cuda/parallel-thread-execution#tensor-dimension-size-format-sub-bytes)[](https://docs.nvidia.com/cuda/parallel-thread-execution/#tensor-dimension-size-format-sub-bytes "Permalink to this headline")
...
![_images/tensor-dimension-size-format-sub-bytes-padding-align-b4-16-p64.png](images/state-spaces-types-and-variables_1.png)
...
![_images/tensor-dimension-size-format-sub-bytes-padding-align-b6-16-p32.png](images/state-spaces-types-and-variables_2.png)
...
![_images/tensor-dimension-size-format-sub-bytes-padding-align-b6-p2-16.png](images/state-spaces-types-and-variables_3.png)
...
### [5.5.3. Tiled Mode](https://docs.nvidia.com/cuda/parallel-thread-execution#tensor-tiled-mode)[](https://docs.nvidia.com/cuda/parallel-thread-execution/#tensor-tiled-mode "Permalink to this headline")
...
A tensor can be accessed in chunks known as _Bounding Box_. The Bounding Box has the same
...
![_images/tensor-tiled-mode-bounding-box-example.png](images/state-spaces-types-and-variables_4.png)
...
![_images/tensor-oob-access.png](images/state-spaces-types-and-variables_5.png)
...
![_images/tiled-scatter4-gather4-bounding-box.png](images/state-spaces-types-and-variables_6.png)
...
_Bounding-box Lower-Corner_ and _Bounding-box Upper-Corner_ specify the two opposite corners of the
Bounding Box in the DHW space. _Bounding-box Lower-Corner_ specifies the corner with the smallest
coordinate and _Bounding-box Upper-Corner_ specifies the corner with the largest coordinate.

_Bounding-box Upper-_ and _Lower-Corners_ are 16-bit signed values whose limits varies across the
dimensions and are as shown below:

|  | 3D | 4D | 5D |
| --- | --- | --- | --- |
| Upper- / Lower- Corner sizes | [-2<sup>15</sup>, 2<sup>15</sup>-1] | [-2<sup>7</sup>, 2<sup>7</sup>-1] | [-2<sup>4</sup>, 2<sup>4</sup>-1] |
...
![_images/tensor-im2col-mode-bounding-box1.png](images/state-spaces-types-and-variables_7.png)

Figure 11 im2col mode bounding box example 1

![_images/tensor-im2col-mode-bounding-box2.png](images/state-spaces-types-and-variables_8.png)

Figure 12 im2col mode bounding box example 2

The _Bounding-box Upper-_ and _Lower- Corners_ specify only the boundaries and not the number of
elements to be accessed. _Pixels-per-Column_ specifies the number of elements to be accessed in the
NDHW space.
...
|  | 3D | 4D | 5D |
| --- | --- | --- | --- |
| im2col offsets range | [0, 2<sup>16</sup>-1] | [0, 2<sup>8</sup>-1] | [0, 2<sup>5</sup>-1] |
...
![_images/tensor-im2col-mode-example1.png](images/state-spaces-types-and-variables_9.png)
...
![_images/tensor-im2col-mode-example2.png](images/state-spaces-types-and-variables_10.png)
...
![_images/tensor-im2col-mode-example3.png](images/state-spaces-types-and-variables_11.png)
...
In im2col mode, when the number of requested pixels in NDHW space specified by _Pixels-per-Column_
...
Note that the valid [Swizzling Modes](https://docs.nvidia.com/cuda/parallel-thread-execution/#tensor-swizzling-modes) must be set. In other words, swizzling mode must not be
...
![_images/tensor-im2col-w-w128-modes-example.png](images/state-spaces-types-and-variables_12.png)
...
![_images/tensor-im2col-w-w128-modes-example2.png](images/state-spaces-types-and-variables_13.png)
...
![_images/tensor-im2col-w-w128-modes-example3.png](images/state-spaces-types-and-variables_14.png)
...
![_images/tensor-im2col-w-w128-modes-example4.png](images/state-spaces-types-and-variables_15.png)
...
![_images/tensor-im2col-w-w128-modes-example5.png](images/state-spaces-types-and-variables_16.png)

Figure 20 tensor copy operation to buffer 1 of Example 1

![_images/tensor-im2col-w-w128-modes-example6.png](images/state-spaces-types-and-variables_17.png)
...
![_images/tensor-im2col-w-w128-modes-example7.png](images/state-spaces-types-and-variables_18.png)

Figure 22 tensor copy operation to buffer 1 of Example 2

![_images/tensor-im2col-w-w128-modes-example8.png](images/state-spaces-types-and-variables_19.png)

Figure 23 tensor copy operation to buffer 2 of Example 2

![_images/tensor-im2col-w-w128-modes-example9.png](images/state-spaces-types-and-variables_20.png)
...
The _C_ information is organized in slices where sequential C elements are grouped in 16 byte or 32
byte quantities.
...
![_images/tensor-32B-swizzle.png](images/state-spaces-types-and-variables_21.png)
...
![_images/tensor-32B-swizzle-frag.png](images/state-spaces-types-and-variables_22.png)
...
![_images/tensor-32B-swizzle-dst.png](images/state-spaces-types-and-variables_23.png)
...
![_images/tensor-64B-swizzle.png](images/state-spaces-types-and-variables_24.png)
...
![_images/tensor-64B-swizzle-src.png](images/state-spaces-types-and-variables_25.png)
...
![_images/tensor-64B-swizzle-dst.png](images/state-spaces-types-and-variables_26.png)
...
![_images/tensor-96B-swizzle.png](images/state-spaces-types-and-variables_27.png)
...
![_images/tensor-128B-swizzle.png](images/state-spaces-types-and-variables_28.png)
...
![_images/tensor-128B-swizzle-src.png](images/state-spaces-types-and-variables_29.png)
...
![_images/tensor-128B-swizzle-dst.png](images/state-spaces-types-and-variables_30.png)
...
![_images/tensor-128B-swizzle-32B-atom.png](images/state-spaces-types-and-variables_31.png)
...
![_images/tensor-128B-swizzle-32B-atom-8B-flip.png](images/state-spaces-types-and-variables_32.png)
...
![_images/tensor-128B-swizzle-64B-atom.png](images/state-spaces-types-and-variables_33.png)
